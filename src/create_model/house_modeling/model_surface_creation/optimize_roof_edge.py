...
def optimize_roof_edge(
    ...
    *,
    ...
    outer_graph = RoofGraph.from_segments(footprint_segments, RoofGraphLabel.OUTER)
    inner_graph = RoofGraph.from_segments(roof_edge_segments, RoofGraphLabel.INNER)
    ...
    inner_graph, outer_graph = optimize_step_02(inner_graph, outer_graph, threshold)
    ...
    graph = optimize_step_06(
        graph, rotate_threshold, same_threshold, rotate_step_size, base_line_vector
    )
    ...
    for segment1 in outer_graph.to_segments():
        ...
        for segment2 in outer_graph.to_segments():
            degree = get_angle_degree(
                Point(0, 0), segment1[1] - segment2[0], segment2[1] - segment2[0]
            )
            if min(degree % 90, 90 - degree % 90) <= threshold:
                sum_length += segment2.length()

        candidates.append((segment1, (sum_length, segment1.length())))
...
def segments_to_polygon(segments: list[Segment]) -> Polygon:
    ...
    for _ in range(len(segments)):
        ...
        next_segment = list(filter(lambda x: cur in x and prev not in x, segments_idx))
        next = list(set(segments_idx[segments_idx.index(next_segment[0])]) - {cur})[0]
        cur, prev = next, cur
...
def optimize_step_01(
    inner_graph: RoofGraph,
    outer_graph: RoofGraph,
    threshold: float,
    point_threshold: float,
    base_line_vector: Point,
):
    ...
    # 垂直、平行の線分の数を数える
    def count_horizontal_vertical_line(
        inner_idx: int, move_to: Point, threshold: float
    ):
        degrees_of_adjacency = [
            get_angle_degree(
                Point(0, 0), inner_graph.nodes[adj] - move_to, base_line_vector
            )
        ...
        return len(
            list(
                filter(
                    lambda deg: min(deg % 90, 90 - deg % 90) < threshold,
                    degrees_of_adjacency,
                )
            )
        )
    ...
    for idx, point in enumerate(inner_graph.nodes):
        ...
        for outer_segment in outer_graph.to_segments():
            ...
            if distance < threshold:
                ...
                # 点からの距離が極端に近い場合は除く
                if (
                    outer_point_set.find_nearest(nearest_point, point_threshold)
                    is not None
                ):
                    continue
        ...
        for candidate in candidates:
            ...
            if all(
                [
                    outer_polygon.intersection(line_str_).length
                    >= line_str_.length * 0.99
                    for line_str_ in line_str_s
                ]
            ):
...
def optimize_step_02(inner_graph: RoofGraph, outer_graph: RoofGraph, threshold: float):
    ...
    for i, j, k in itertools.combinations(range(inner_graph.node_size), 3):
        ...
        for a, b, c in itertools.permutations([i, j, k], 3):
            ...
            # a-c------b を (ac)------b とする
            if point_c.distance(point_a) < threshold and point_c.distance(
                point_a
            ) <= point_c.distance(point_b):
                base_edge = (a, b)
        ...
        if base_edge and moving_point:
            ...
            inner_graph.add_edge(base_edge[0], moving_point[0], RoofGraphLabel.INNER)
            inner_graph.add_edge(base_edge[1], moving_point[0], RoofGraphLabel.INNER)
...
def optimize_step_03(graph: RoofGraph, extension_length: float):
    ...
    for a, b, _ in graph.edge_list(label=RoofGraphLabel.INNER):
        if graph.nodes[a].is_same(
            graph.nodes[b]
        ):  # ノードを移動するため、同じ位置になる場合がある
            continue
        ...
        if len(candidates_a) and graph.degree(a) == 1:
            new_point_a = min(candidates_a, key=lambda point: point_a.distance(point))
            graph.move_node(a, new_point_a, permit_to_merge=False)
        if len(candidates_b) and graph.degree(b) == 1:
            new_point_b = min(candidates_b, key=lambda point: point_b.distance(point))
...
def optimize_step_04(graph: RoofGraph, extension_length: float):
    ...
    for target_idx, target in enumerate(graph.nodes):
        ...
        adjacencies = graph.get_adjacencies(target_idx, label=RoofGraphLabel.INNER)
        # assert len(adjacencies) == 1, f"次数1の外形線の頂点が存在します"

        # 他の線分上にある場合は除く
        disjoint_segments = list(
            filter(
                lambda segment: segment[0] != target_idx and segment[1] != target_idx,
                graph.edge_list(),
            )
        )
        is_on_other_segment = any(
            [
                target.is_on(Segment(graph.nodes[a], graph.nodes[b]))
        ...
        if len(candidates):
            ...
            graph.add_edge(target_idx, added_node_idx, label=RoofGraphLabel.INNER)
...
def optimize_step_05(graph: RoofGraph, threshold: float, same_threshold: float):
        ...
        def filter_func(adj):
            ...
            filter_1_result = all(
                [
                    ...
                    # 線分の外側に位置していないか
                    max(point_adj.distance(point_a), point_adj.distance(point_b))
    ...
    for a, b, _ in graph.edge_list(label=RoofGraphLabel.INNER):
        ...
        adjacencies_a = set(graph.get_adjacencies(a, label=RoofGraphLabel.OUTER))
        adjacencies_b = set(graph.get_adjacencies(b, label=RoofGraphLabel.OUTER))

        # 線分ab上にある点を抽出 (距離が少し離れていても許容する)
        adjacencies_on_ab = list(
            set(
                filter(
                    filter_func_gen(
                        graph, point_a, point_b, same_threshold, segment_ab, threshold
                    ),
...
def optimize_step_06(
    ...
    for a, b, _ in graph.edge_list(label=RoofGraphLabel.INNER):
        ...
        degree: float = get_angle_degree(
            Point(0, 0), point_a - point_b, base_line_vector
        )
        ...
        new_degree = degree - (
            degree % step_size
            if degree % step_size < step_size / 2
        ...
        # 移動前の線分上にあった点を移動する
        inner_only_points = set(
            filter(
                lambda idx: graph.degree(idx, label=RoofGraphLabel.INNER) > 0
                and graph.degree(idx, label=RoofGraphLabel.OUTER) == 0,
        ...
        for inner_point_idx in inner_only_points - {a, b}:
            ...
            graph.move_node(inner_point_idx, new_inner_point, permit_to_merge=False)
...
def optimize_step_07(graph: RoofGraph):
    ...
    for a, b, label in graph.edge_list():
        ...
        for u, v, _ in list(graph.edge_list()):
            ...
            if (
                cross_point
                and not segment[0].is_same(cross_point)
                and not segment[1].is_same(cross_point)
            ):
                graph.add_node(cross_point)  # 既存の点と重複する場合には追加されない

        points_on_segment = list(
            filter(lambda idx: graph.nodes[idx].is_on(segment), range(graph.node_size))
        )

        points_on_segment.sort(key=lambda idx: graph.nodes[idx].distance(segment[0]))
...
def optimize_step_10(graph: RoofGraph, straight_threshold: float):
    ...
    for idx in range(graph.node_size):
        ...
        is_straight = (
            abs(get_angle_degree(graph.nodes[a], graph.nodes[idx], graph.nodes[b]))
            < straight_threshold
        )
