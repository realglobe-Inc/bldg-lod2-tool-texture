...
def plane_graph_to_polygons(graph: RoofGraph) -> tuple[list[int], list[list[int]]]:
    ...
    for i in range(v):
        ...
        adjacencies[i].sort(
            key=lambda adj: get_angle_degree(
                graph.nodes[i],
    ...
    is_used_edge: list[list[bool]] = [[False] * len(adjacencies[i]) for i in range(v)]
...
def add_hole(polygon: list[int], hole: list[int], points: list[Point]) -> list[int]:
    ...
    for i, polygon_point in enumerate(polygon):
        for j, hole_point in enumerate(hole):
            ...
            if (
                not polygon_shape.contains(line)
                or hole_shape.intersection(line).length > 0
            ):
                continue
...
def to_shapely_polygon(polygon: list[int], points: list[Point]) -> Polygon:
    ...
    """
    return Polygon([(points[id].x, points[id].y) for id in polygon])
