...
from .....third_party.plateaupy.third_party.earcut_python.earcut.earcut import earcut
...
def triangulation(
    ...
    assert points.ndim == 2 and points.shape[1] == 3, "shape of points must be (*, 3)"
        ...
        for j in range(i, n):
            if polygon[i] == polygon[j]:
                ...
                for edge in pairwise(range(n), loop=True):
                    # 頂点が同じ場合は除く
                    if len(
                        {polygon[i], polygon[j]} & {polygon[edge[0]], polygon[edge[1]]}
                    ):
                        continue
        ...
        for left in range(0, 2 * n - delta):
            ...
            for center in range(left + 1, right):
                ...
                if left + 1 == center:
                    ...
                    dot_values = np.dot(
                        normals[left, :, center], normals[left, center, right]
                    )
                ...
                if center + 1 == right:
                    ...
                    dot_values = np.dot(
                        normals[center, :, right], normals[left, center, right]
                    )
                    angle_rad = np.arccos(np.clip(dot_values, -1, 1))
...
def triangulation_2d(
    ...
    for delta in range(2, n):
        ...
        costs = (dp[:-delta, :] + dp[:, delta:].T) + np.diag(distance, delta)[
            :, np.newaxis
        ]
...
def triangulate(polygon: list[int], vertex_xy: list[tuple[float, float]]):
    ...
    polygon_xys: list[tuple[float, float]] = [
        vertex_xy[point_id] for point_id in polygon
    ]
    ...
    if len(polygon_xys) == 3:
        triangle = Triangle(
            [
                TriangleVertex(point_id, order_id)
                for order_id, point_id in enumerate(polygon)
            ]
        )
        return [triangle]
    ...
    for other_poly in other_polys:
        ...
        if len(other_poly_xys) == 3:
            ...
            earcut_res = earcut(
                np.array(other_poly_xys, dtype=np.float64).flatten(), dim=2
            )
            other_poly_earcut_triangles: list[list[int]] = (
                np.array(earcut_res).reshape((-1, 3)).tolist()
            )
            for other_poly_earcut_triangle in other_poly_earcut_triangles:
                split_triangle = Polygon(
                    [
                        other_poly_xys[cutted_order_id]
                        for cutted_order_id in other_poly_earcut_triangle
                    ]
                )
                other_poly_triangles.append(split_triangle)
