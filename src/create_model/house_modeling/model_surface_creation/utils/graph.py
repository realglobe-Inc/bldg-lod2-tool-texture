...
class RoofGraph:
    ...
    __edges: npt.NDArray[
        np.int8
    ]  # 隣接行列 (隣接していない場合は-1, 隣接している場合はラベル番号)
    nodes: list[Point]
    ...
    def move_node(self, node_idx: int, to: Point, permit_to_merge=False) -> None:
        ...
        if same_position_node and same_position_node != node_idx and permit_to_merge:
            ...
            self.__edges[same_position_node, mask] = self.__edges[node_idx][mask]
            self.__edges[mask, same_position_node] = self.__edges[node_idx][mask]
    ...
    def add_edge(
        ...
        node_idx_1: int,
        node_idx_2: int,
        label: RoofGraphLabel = RoofGraphLabel.DEFAULT,
    ) -> None:
        """エッジの追加

        Args:
            node_idx_1(int): 追加するエッジの頂点
            node_idx_2(int): 追加するエッジの頂点
        ...
        if node_idx_1 != node_idx_2:
            self.__edges[node_idx_1, node_idx_2] = max(
                self.__edges[node_idx_1, node_idx_2], label
            )
            self.__edges[node_idx_2, node_idx_1] = max(
                self.__edges[node_idx_2, node_idx_1], label
            )
    ...
    def delete_edge(self, node_idx_1: int, node_idx_2: int) -> None:
        ...
        Args:
            node_idx_1(int): 削除するエッジの頂点
            node_idx_2(int): 削除するエッジの頂点
        """
        self.__edges[node_idx_1, node_idx_2] = -1
        self.__edges[node_idx_2, node_idx_1] = -1

    def get_edge_label(
        self, node_idx_1: int, node_idx_2: int
    ) -> Optional[RoofGraphLabel]:
        """2頂点間にあるエッジのラベルを取得する

        Args:
            node_idx_1(int): 取得するエッジの頂点
            node_idx_2(int): 取得するエッジの頂点
    ...
    def edge_list(
        self, label: Optional[RoofGraphLabel] = None
    ) -> Generator[tuple[int, int, RoofGraphLabel], None, None]:
        """エッジの一覧のジェネレータを取得する
        ...
        Returns:
            Generator[tuple[int, int, RoofGraphLabel], None, None]: (頂点1,頂点2,ラベル)のタプルを生成するジェネレータ
        ...
        for i, j in itertools.product(range(num_of_nodes), repeat=2):
            ...
            if (label is None and self.__edges[i, j] != -1) or self.__edges[
                i, j
            ] == label:
    ...
    def to_segments(self, label: Optional[RoofGraphLabel] = None) -> list[Segment]:
        ...
        """
        return [
            Segment(self.nodes[a], self.nodes[b])
            for a, b, _ in self.edge_list(label=label)
        ]
    ...
    def get_adjacencies(
        self, node_idx: int, label: Optional[RoofGraphLabel] = None
    ) -> list[int]:
        """隣接頂点を取得する
    ...
    def simplify(self, delete_unconnected_nodes=False):
        ...
        new_edges = np.full((len(new_nodes), len(new_nodes)), -1, dtype=np.int8)

        for i, j in itertools.product(range(len(self.__edges)), repeat=2):
            if new_indices[i] != new_indices[j] and self.__edges[i, j] != -1:
                new_edges[new_indices[i], new_indices[j]] = max(
                    new_edges[new_indices[i], new_indices[j]], self.__edges[i, j]
                )
    ...
    def merge(graphs: list[RoofGraph]) -> RoofGraph:
        ...
        nodes = list(itertools.chain.from_iterable([g.nodes for g in graphs]))
        ...
        for g in graphs:
            for a, b, label in g.edge_list():
                graph.add_edge(a + index_begin, b + index_begin, RoofGraphLabel(label))
    ...
    def from_segments(
        ...
        labels: Optional[Union[list[RoofGraphLabel], RoofGraphLabel]] = None,
    ):
