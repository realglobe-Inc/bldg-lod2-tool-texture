...
class HouseModelBuilder:
    ...
    def __init__(
        ...
        self._balcony_segmentation_checkpoint_path = (
        ...
        (
            self._square_dsm_grid_rgb,
            self._depth_image,
            self._roof_layer_info,
        ) = preprocess.preprocess(self._cloud, min_ground_height, shape, debug_mode)

        self._dsm_coord_heat_image_pos_converter = (
            self._get_dsm_coord_heat_image_pos_converter()
        )
        roof_edge_detection = RoofEdgeDetection(
            self._roof_edge_detection_checkpoint_path, self._use_gpu
        )
        tmp_roof_edge_vertice_ijs, tmp_roof_edges = roof_edge_detection.infer(
            self._square_dsm_grid_rgb
        )
        ...
        tmp_roof_vertex_xys = np.array(
            [
                self._dsm_coord_heat_image_pos_converter.image_point_to_cartesian_point(
                    i, j
                )
        ...
        # ポリゴンがバルコニーか
        polygon_balcony_flags = self._get_polygon_balcony_flags(
            roof_polygon_vertex_xy_points, inner_polygons
        )
        polygon_balcony_flags = [False for _ in polygon_balcony_flags]

        # バルコニーの屋根レイヤーを追加
        balcony_polygon_ijs_list = self._get_balcony_polygon_ijs_list(
            polygon_balcony_flags,
            roof_polygon_vertex_ijs,
            inner_polygons,
        )
    ...
    def _get_dsm_coord_heat_image_pos_converter(self):
        ...
        expanded_grid_size = self._grid_size / (
            self._expand_rate if self._expand_rate is not None else 1
        )
    ...
    def _get_polygon_balcony_flags(
        ...
        image_points = [
            Point(
                *self._dsm_coord_heat_image_pos_converter.cartesian_point_to_image_point(
                    cartesian_point.x, cartesian_point.y
                )
            )
        ...
        balcony_detection = BalconyDetection(
            self._balcony_segmentation_checkpoint_path, self._use_gpu
        )
        balcony_flags = balcony_detection.infer(
            dsm_grid_rgb=self._square_dsm_grid_rgb,
    ...
    def _get_balcony_polygon_ijs_list(
        ...
        for polygon_id, polygon_balcony_flag in enumerate(polygon_balcony_flags):
            if polygon_balcony_flag:
                ...
                balcony_polygon_ijs = [
                    roof_polygon_vertex_ijs[point_id] for point_id in balcony_polygon
                ]
    ...
    def _get_roof_polygon_vertex_ijs(
        ...
        for polygon in polygons:
            polygon_ijs = [
                fixed_roof_polygon_vertex_ijs[point_id] for point_id in polygon
            ]
    ...
    def _patch_for_heat_polygon(
        ...
        tmp_roof_polygon_vertex_ijs = [
            self._xy_to_ij((xy_point.x, xy_point.y))
            for xy_point in tmp_roof_polygon_vertex_xy_points
        ]

        inner_polygon_ijs_list_before: list[list[tuple[int, int]]] = []
        for inner_polygon in tmp_inner_polygons:
            inner_polygon_ijs = [
                tmp_roof_polygon_vertex_ijs[point_id] for point_id in inner_polygon
            ]
        ...
        if extra_roof_line.has_split_polygon:
            roof_polygon_vertex_xy_points = [
