...
from .extra_roof_line.polygon_devision import PolygonDivision
...
class ModelEdgeHeightInfo:
    ...
    def __init__(
        ...
        # ポリゴンそれぞれの屋根レイヤー
        self._polygon_layer_numbers = self._get_polygon_layer_numbers(
            self._layer_number_point_ijs_pairs
        )

        # 頂点 -> 高さ 検索時に使う
        self._point_id_polygon_layer_zs_pair = (
            self._get_point_id_polygon_layer_zs_pair()
        )
        ...
        # 内部ポリゴン -> エッジ 検索時に使う
        self._polygon_id_inner_polygon_sorted_edges_pair: dict[
        ...
        for polygon_id, inner_polygon in enumerate(self._inner_polygons):
            ...
            self._polygon_id_inner_polygon_sorted_edges_pair[polygon_id] = (
                inner_polygon_sorted_edges
            )

        # エッジ -> 内部ポリゴン 検索時に使う
        self._polygon_sorted_edge_polygon_ids_pair: dict[tuple[int, int], list[int]] = (
            defaultdict(list)
        )
        for (
            polygon_id,
            inner_polygon_sorted_edges,
        ) in self._polygon_id_inner_polygon_sorted_edges_pair.items():
            for inner_polygon_sorted_edge in inner_polygon_sorted_edges:
                self._polygon_sorted_edge_polygon_ids_pair[
                    inner_polygon_sorted_edge
                ].append(polygon_id)
    ...
    def _get_layer_number_point_ijs_pairs(self):
        ...
        for inner_polygon in self._inner_polygons:
            polygon_ijs = [
                self._roof_polygon_vertex_ijs[point_id] for point_id in inner_polygon
            ]
    ...
    def _get_polygon_layer_numbers(
        ...
        Args:
          layer_number_point_ijs_pairs (list[dict[int, list[tuple[float, float]]]]): ポリゴン毎にポリゴン内部領域に入っている頂点(i,j)の屋根レイヤーを出したもの
    ...
    def _get_point_id_polygon_layer_zs_pair(self):
        # 頂点 -> 高さ 検索時に使う
        point_id_polygon_layer_zs_pair: dict[int, dict[int, dict[int, float]]] = (
        ...
        for polygon_id, inner_polygon in enumerate(self._inner_polygons):
            ...
            for point_id in inner_polygon:
                ...
                if is_balcony_polygon:
                    ...
                    balcony_z = (
                        self._get_balcony_z(polygon_id)
                        if is_balcony_polygon
                        else self._min_roof_height
                    )
                    point_id_polygon_layer_zs_pair[point_id][polygon_layer_number][
                        polygon_id
                    ] = balcony_z
                else:
                    # 頂点と一番近い頂点をの座標(i,j)をポリゴン内部から検索
                    point_ij = self._roof_polygon_vertex_ijs[point_id]
                    nearest_layer_number_point_ij = find_closest_point(
                        point_ij, available_polygon_ijs
                    )
                    # ポリゴンの頂点での壁の高さを取得
                    nearest_z = (
                        self._roof_layer_info.ij_to_z(*nearest_layer_number_point_ij)
                        if nearest_layer_number_point_ij is not None
                        else 0
                    )
    ...
    @staticmethod
    def _get_polygon_sorted_edges(polygon: list[int]):
        """ポリゴンのエッジリストを取得
        ...
        for (
            ...
            for inner_polygon_sorted_edge in inner_polygon_sorted_edges:
                ...
                if inner_polygon_sorted_edge in self._outer_polygon_sorted_edges:
                    ...
                    average_z1 = statistics.mean(
                        self._point_id_polygon_layer_zs_pair[point_id1][
                            polygon_layer
                        ].values()
                    )
                    average_z2 = statistics.mean(
                        self._point_id_polygon_layer_zs_pair[point_id2][
                            polygon_layer
                        ].values()
                    )
                    sorted_edge_wall_bottom_top_pair[inner_polygon_sorted_edge] = (
                        (self._ground_height, average_z1),
                        (self._ground_height, average_z2),
                    )
                    continue

                # エッジを共有しているポリゴンは二つ
                polygon_id1, polygon_id2 = self._polygon_sorted_edge_polygon_ids_pair[
                    inner_polygon_sorted_edge
                ]
                ...
                if polygon_layer1 != polygon_layer2:
                    # その頂点で屋根レイヤーの高さ平均
                    average_z1_1 = statistics.mean(
                        self._point_id_polygon_layer_zs_pair[point_id1][
                            polygon_layer1
                        ].values()
                    )
                    average_z1_2 = statistics.mean(
                        self._point_id_polygon_layer_zs_pair[point_id1][
                            polygon_layer2
                        ].values()
                    )
                    ...
                    average_z2_1 = statistics.mean(
                        self._point_id_polygon_layer_zs_pair[point_id2][
                            polygon_layer1
                        ].values()
                    )
                    average_z2_2 = statistics.mean(
                        self._point_id_polygon_layer_zs_pair[point_id2][
                            polygon_layer2
                        ].values()
                    )
    ...
    def _get_polygon_zs_list(self):
        ...
        for polygon_id, inner_polygon in enumerate(self._inner_polygons):
            ...
            polygon_layer_number = self._polygon_layer_numbers[polygon_id]
            for point_id in inner_polygon:
                average_z = statistics.mean(
                    self._point_id_polygon_layer_zs_pair[point_id][
                        polygon_layer_number
                    ].values()
                )
    ...
    def _get_twisted_edge_middle_point_rate_pair(
        ...
        """
        twisted_edge_middle_point_rate_pair: dict[
            tuple[int, int], tuple[float, float]
        ] = {}
        for (
            inner_polygon_sorted_edges
        ) in self._polygon_id_inner_polygon_sorted_edges_pair.values():
            for inner_polygon_sorted_edge in inner_polygon_sorted_edges:
                ...
                if twisted_edge_zs is not None:
                    ...
                    twisted_edge_middle_point_rate_pair[inner_polygon_sorted_edge] = (
                        point_rate1
                    )
    ...
    def _get_balcony_z(self, polygon_id: int):
        ...
        grid_point_zs = [
            self._roof_layer_info.ij_to_z(*grid_point_ij)
            for grid_point_ij in grid_point_ijs
        ]
    ...
    def _fix_twisted_edge(
        ...
        """
        twisted_edge_middle_point_rate_pair = (
            self._get_twisted_edge_middle_point_rate_pair(
        ...
        fixed_sorted_edge_wall_bottom_top_pair = copy.deepcopy(
            sorted_edge_wall_bottom_top_pair
        )
        ...
        for (
            twisted_edge,
            middle_point_rate,
        ) in twisted_edge_middle_point_rate_pair.items():
            ...
            middle_point_xy = self._calculate_midpoint(
                edge_xy1, edge_xy2, middle_point_rate
            )
            middle_point_ij = self._calculate_midpoint(
                edge_ij1, edge_ij2, middle_point_rate
            )
            ...
            for polygon_id in polygon_ids:
                ...
                if max_point_index - min_point_index == 1:
                    fixed_polygon_zs_list[polygon_id].insert(
                        min_point_index + 1, middle_point_z
                    )
                    fixed_inner_polygons[polygon_id].insert(
                        min_point_index + 1, new_point_id
                    )
                else:
