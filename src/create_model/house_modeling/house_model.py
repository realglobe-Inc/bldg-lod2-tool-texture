...
class ModelFace:
    ...
    def edges_2d(self):
        return pairwise([point.position_id_2d for point in self.points], loop=True)

    def edges_3d(self):
        return pairwise([point.position_id_3d for point in self.points], loop=True)
...
class HouseModel:
    ...
    def __init__(
        ...
        self._fixed_roof_polygon_vertex_xys = (
            model_edge_height_info.fixed_roof_polygon_vertex_xys
        )
        self._fixed_roof_polygon_vertex_ijs = (
            model_edge_height_info.fixed_roof_polygon_vertex_ijs
        )
    ...
    def _add_point(
        self, position: Union[tuple[float, float, float], npt.NDArray[np.float_]]
    ) -> int:
        """点を追加する
    ...
    def _create_wall_faces(
        ...
        for polygon_id, polygon in enumerate(self._fixed_inner_polygons):
            polygon_xys_before = [
                self._fixed_roof_polygon_vertex_xys[point_id] for point_id in polygon
            ]
            ...
            for index, point_id in enumerate(roof_polygon):
                ...
                # 壁だけ作る
                wall_bottom_top = fixed_sorted_edge_wall_bottom_top_pair.get(
                    sorted_edge
                )
    ...
    def _create_roof_faces(self, fixed_polygon_zs_list: list[list[float]]):
        ...
        for polygon_id, triangles in polygon_id_triangles_pair.items():
            ...
            for triangle in triangles:
                ...
                triangle_xys_before = [
                    self._fixed_roof_polygon_vertex_xys[triangle_vertex.point_id]
    ...
    def _create_ground_face(self):
        ...
        ground_polygon_xys_before = [
            self._fixed_roof_polygon_vertex_xys[point_id]
            for point_id in self._outer_polygon
        ]
    ...
    def _get_polygon_id_triangles_pair(
        ...
        if self._debug_mode:
            ...
            triangulation_before_polygons_2d.append_faces(
                BldElementType.ROOF, polygon_layer_xys_list
            )
            triangulation_before_triangles_2d.append_faces(
                BldElementType.ROOF, triangle_layer_xys_list
            )

            debug_dir = os.path.join("debug", self._id)
            Path(debug_dir).mkdir(parents=True, exist_ok=True)
            triangulation_before_polygons_2d_obj_path = os.path.join(
                debug_dir, "triangulation_before_polygons_2d.obj"
            )
            triangulation_before_triangles_2d_obj_path = os.path.join(
                debug_dir, "triangulation_before_triangles_2d.obj"
            )

            triangulation_before_polygons_2d.write_file(
                file_path=triangulation_before_polygons_2d_obj_path
            )
            triangulation_before_triangles_2d.write_file(
                file_path=triangulation_before_triangles_2d_obj_path
            )
        ...
        assert isinstance(
            triangle_poly_area, Polygon
        ), "分割された三角形に隙間があります"
        assert (
            len(triangle_poly_area.interiors) == 0
        ), "分割された三角形に隙間があります"
    ...
    def _rectify(self):
        ...
        def find_on_segment_point(edge_3d: tuple[int, int], skip_list: set[int]) -> int:
            # 線分の頂点
        ...
        # 連続点を探索
        db = DBSCAN(eps=1e-02, min_samples=1)
        db.fit(self._points)
        labels = db.labels_
        ...
        for ci in range(n_clusters):
            ...
            point_groups.append(indices.tolist())
        ...
        for face in self._faces:
            ...
            rectified_faces.append(
                ModelFace(
                    [
                        ModelPoint(-1, position_id, -1)
        ...
        for face in self._faces:
            ...
            while True:
                ...
                for edge_3d in edges_3d:
                    i = find_on_segment_point(edge_3d, skip_list)
                    if i >= 0:
    ...
    @staticmethod
    def _to_polygon(direct_edges: list[tuple[int, int]]) -> list[int]:
        """有向辺から多角形を復元する
    ...
    def output_obj(self, path: str):
        ...
        info = ObjInfo()
        info.append_faces(
            BldElementType.ROOF, [self._points[roof.position_ids_3d] for roof in roofs]
        )
        info.append_faces(
            BldElementType.WALL, [self._points[wall.position_ids_3d] for wall in walls]
        )
        info.append_faces(
            BldElementType.GROUND,
