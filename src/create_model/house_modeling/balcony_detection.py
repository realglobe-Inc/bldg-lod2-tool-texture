...
class BalconyDetection:
    ...
    def infer(
        self,
        dsm_grid_rgb: npt.NDArray[np.uint8],
        ...
        Args:
            dsm_grid_rgb(NDArray[np.uint8]): (image_size, image_size, 3)のRGB画像データ
        ...
        x = np.concatenate(
            [dsm_grid_rgb[:, :, ::-1], depth_image[:, :, np.newaxis]],  # RGB -> BGR
            axis=2,
        ).astype(np.float32)
        ...
        with torch.inference_mode():
            ...
            pred = predicates.detach().cpu().numpy()[0].astype(np.bool8)

        balcony_flags: list[bool] = []

        # 各polygonのバルコニー判定を行う
        for polygon in polygons:
            mask_image = Image.new("1", (self._image_size, self._image_size), 0)
