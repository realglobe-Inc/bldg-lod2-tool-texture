...
from .polygon_devision import PolygonDivision
...
from ....create_model.create_model_exception import ModelingException
from ....create_model.message import ModelingMessage
...
class ExtraRoofLine:
    ...
    def __init__(
        ...
        self._inner_polygon_ijs_list_before: list[list[tuple[float, float]]] = [
            ensure_counter_clockwise(inner_polygon_ijs)
            for inner_polygon_ijs in inner_polygon_ijs_list_before
        ]
        ...
        self._new_polygon_vertex_ijs = list(
            set(
                [
                    polygon_ij
        ...
        self._new_polygon_vertex_xys = [
            self._ij_to_xy(ij) for ij in self._new_polygon_vertex_ijs
        ]

        vertex_xy_point_id_pair = {
            vertex_ij: index
            for index, vertex_ij in enumerate(self._new_polygon_vertex_xys)
        }
        ...
        for inner_polygon_xys in self._inner_polygon_xys_list:
            self._new_inner_polygons.append(
                [
                    vertex_xy_point_id_pair[inner_polygon_xy]
                    for inner_polygon_xy in inner_polygon_xys
                ]
            )
        ...
        if self._debug_mode:
            self._save_roof_line_with_layer_class_images(
                self._inner_polygon_ijs_list_after,
                "roof_line_with_layer_class_step_5_split_polygons.png",
                "roof_line_with_layer_class_step_6_split_roof_layers.png",
                "roof_line_with_layer_class_step_7_filled_split_polygons.png",
            )

    def _has_too_many_noise(self):
        has_too_many_noise = False
        for polygon_ijs in self._inner_polygon_ijs_list_after:
            layer_number_point_ijs_pair = (
                PolygonDivision.get_layer_number_grid_ijs_pair(
                    self._roof_layer_info, polygon_ijs
                )
            )
            noise_point_ijs = (
                layer_number_point_ijs_pair.get(RoofLayerInfo.NOISE_POINT) or []
            )
            ...
            if (
                point_ijs_count_all >= 25
                and (point_ijs_count_noise / point_ijs_count_all) > 0.4
            ):
    ...
    def _save_roof_line_with_layer_class_images(
        ...
        # RGBイメージの初期化
        dsm_grid_rgb_images_of_roof_line_with_layer_class_as_is = np.full(
            (self._height, self._width, 3), 255, dtype=np.uint8
        )
        dsm_grid_rgb_images_of_roof_line_with_layer_class_to_be = np.full(
            (self._height, self._width, 3), 255, dtype=np.uint8
        )
        ...
        for polygon_ijs in inner_polygon_ijs_list:
            ...
            for layer_number, layer_points_ij in layer_number_point_ijs_pair.items():
                for i, j in layer_points_ij:
                    dsm_grid_rgb_images_of_roof_line_with_layer_class_as_is[i, j] = (
                        self._roof_layer_info.get_color(layer_number)
                    )
                    dsm_grid_rgb_images_of_roof_line_with_layer_class_to_be[i, j] = (
                        self._roof_layer_info.get_color(majority_layer_number)
                    )

        self._roof_layer_info.save_roof_line_image(
            self._roof_layer_info.masked_dsm_grid_rgb_image,
            polygon_edges_for_debug_image,
            file_name_1,
        )
        self._roof_layer_info.save_roof_line_image(
            dsm_grid_rgb_images_of_roof_line_with_layer_class_as_is,
            polygon_edges_for_debug_image,
            file_name_2,
        )
        self._roof_layer_info.save_roof_line_image(
            dsm_grid_rgb_images_of_roof_line_with_layer_class_to_be,
            polygon_edges_for_debug_image,
            file_name_3,
        )

    def _get_inner_polygon_ijs_list_after(
        self, inner_polygon_ijs_list_before: list[list[tuple[float, float]]]
    ):
        """
        ...
        for polygon_ijs_before in inner_polygon_ijs_list_before:
            polygon_division = PolygonDivision(
                polygon_ijs_before, self._roof_layer_info, self._debug_mode
            )
            tmp_inner_polygon_ijs_list_after.extend(
                polygon_division.splited_polygon_ijs_list
            )

            if polygon_division.has_splited_polygon:
                self._has_split_polygon = True

                polys = get_polys_from_polygon_ijs_list(
                    polygon_division.splited_polygon_ijs_list
                )
                polys_area = unary_union(polys)

                # ポリゴンが分割された場合、新しい頂点が生成される。外注ポリゴンを取得
                split_polygon_outer_ijs: list[tuple[float, float]] = (
                    ensure_counter_clockwise(list(polys_area.exterior.coords[:-1]))
                )
                ...
                for index, polygon_ij in enumerate(polygon_ijs_before):
                    ...
                    split_polygon_outer_ij_index = split_polygon_outer_ijs.index(
                        polygon_ij
                    )
                    next_split_polygon_outer_ij_index = split_polygon_outer_ijs.index(
                        next_polygon_ij
                    )
                    index_distance = abs(
                        next_split_polygon_outer_ij_index - split_polygon_outer_ij_index
                    )

                    if index_distance != 1 or index_distance != (
                        len(split_polygon_outer_ijs) - 1
                    ):
                        start_index = (split_polygon_outer_ij_index + 1) % len(
                            split_polygon_outer_ijs
                        )
                        end_index = next_split_polygon_outer_ij_index
                        curren_index = start_index

                        while curren_index != end_index:
                            new_polygon_outer_ij = split_polygon_outer_ijs[curren_index]

                            # エッジを基準に、追加された頂点を他のポリゴンにも追加する
                            edge_added_new_point_ijs_pair[sorted_edge].append(
                                new_polygon_outer_ij
                            )
                            curren_index = (curren_index + 1) % len(
                                split_polygon_outer_ijs
                            )
        ...
        for before_polygon_ijs in tmp_inner_polygon_ijs_list_after:
            ...
            for index, before_polygon_ij in enumerate(before_polygon_ijs):
                ...
                if new_point_ijs:
                    is_same_index_direction = sorted_edge == (
                        before_polygon_ij,
                        next_before_polygon_ij,
                    )
                    sorted_new_point_ijs = (
                        sorted(new_point_ijs)
                        if is_same_index_direction
                        else reversed(sorted(new_point_ijs))
                    )
        ...
        tmp_outer_polygon_edges: list[
            tuple[tuple[float, float], tuple[float, float]]
        ] = []
        for index, coord in enumerate(tmp_polys_area.exterior.coords):
            next_coord = tmp_polys_area.exterior.coords[
                (index + 1) % len(tmp_polys_area.exterior.coords)
            ]
            sorted_edge: tuple[tuple[float, float], tuple[float, float]] = tuple(
                sorted([coord, next_coord])
            )
        ...
        for tmp_inner_polygon_ijs in tmp_inner_polygon_ijs_list_after2:
            ...
            for index, coord in enumerate(tmp_inner_polygon_ijs):
                ...
                next_line_group_key = tmp_line_group.get_line_group_key(
                    next_coord, next_next_coord
                )
        ...
        for tmp_inner_polygon_ijs in tmp_inner_polygon_ijs_list_after3:
            ...
            for index, inner_polygon_ij in enumerate(tmp_inner_polygon_ijs):
                ...
                line_point_ijs = tmp_line_group2.get_line_group(
                    inner_polygon_ij, next_inner_polygon_ij
                )
                if line_point_ijs is not None:
                    ...
                    for sorted_point_ij_index in range(
                        start_index, end_index, range_direction
                    ):
                        new_inner_polygon_ij = sorted_line_point_ijs[
                            sorted_point_ij_index
                        ]
        ...
        merged_inner_polygon_ijs_list_after = merge_close_vertices(
            inner_polygon_ijs_list_after
        )

        if self._debug_mode:
            extra_roof_line_polygons_2d = ObjInfo()
            for polygon_idx, tmp_inner_polygon_ijs in enumerate(
                merged_inner_polygon_ijs_list_after
            ):
                polygon_layer_ijs: list[tuple[float, float, float]] = []
                for i, j in tmp_inner_polygon_ijs:
                    polygon_layer_ijs.append((i, j, 2 * polygon_idx))
                extra_roof_line_polygons_2d.append_faces(
                    BldElementType.ROOF, [polygon_layer_ijs]
                )

            debug_dir = os.path.join("debug", self._id)
            Path(debug_dir).mkdir(parents=True, exist_ok=True)
            extra_roof_line_polygons_2d_path = os.path.join(
                debug_dir, "extra_roof_line_polygons_2d.obj"
            )
            extra_roof_line_polygons_2d.write_file(
                file_path=extra_roof_line_polygons_2d_path
            )
    ...
    def _ij_to_xy(self, ij: tuple[float, float]):
        ...
        j_height, i_width = self._roof_layer_info.masked_dsm_grid_rgb_image.shape[:2]
    ...
    @staticmethod
    def _get_line_group(inner_polygon_ijs_list: list[list[tuple[float, float]]]):
        """傾きと切片で同じ直線上にあるエッジの頂点をグループ化
