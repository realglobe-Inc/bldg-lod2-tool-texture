...
class LineGroup:
    def __init__(self, distance_threshold: float = 1e-4):
        # 各グループキーに属する点を管理
        self._line_group_key_point_ijs_pair: dict[
            tuple[float, float], set[tuple[float, float]]
        ] = defaultdict(set)
    ...
    def get_line_group_key(
        ...
        for existing_line_group_key in self._line_group_key_point_ijs_pair:
            ...
            start_point_distance = self._calculate_normal_distance(
                existing_a, existing_b, start_point_ij
            )
            end_point_distance = self._calculate_normal_distance(
                existing_a, existing_b, end_point_ij
            )

            if (
                start_point_distance < self._distance_threshold
                and end_point_distance < self._distance_threshold
            ):
    ...
    def _calculate_normal_distance(
        ...
        if existing_a != float("inf"):
            # 通常の直線の場合
            distance = abs(existing_a * i - j + existing_b) / (
                (existing_a**2 + 1) ** 0.5
            )
        else:
            # 垂直線の場合
