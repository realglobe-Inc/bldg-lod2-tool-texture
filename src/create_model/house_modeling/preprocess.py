...
class Preprocess:
    ...
    def preprocess(
        ...
        nn.fit(pc_xyz[:, 0:2])
        indices = nn.kneighbors(xy, return_distance=False)[:, 0]

        origin_dsm_grid_xyz = pc_xyz[indices]
        masked_dsm_grid_xyz = pc_xyz[indices]
        masked_dsm_grid_rgb = pc_rgb[indices] / 256

        lower, upper = ground_height - 5, ground_height + 25
        masked_depth_image = (
            (np.clip(pc_xyz[:, 2][indices], lower, upper) - lower)
            / (upper - lower)
            * 255
        )
        ...
        for i, (x, y) in enumerate(xy):
            ...
            if not footprint.contains(p):
                masked_dsm_grid_xyz[i] = 0
                masked_dsm_grid_rgb[i] = 255
                masked_depth_image[i] = 255

        origin_dsm_grid_xyz = origin_dsm_grid_xyz.reshape(height, width, 3).astype(
            np.float_
        )
        masked_dsm_grid_xyz = masked_dsm_grid_xyz.reshape(height, width, 3).astype(
            np.float_
        )
        masked_dsm_grid_rgb = masked_dsm_grid_rgb.reshape(height, width, 3).astype(
            np.uint8
        )
        ...
        roof_layer_info = RoofLayerInfo(
            origin_dsm_grid_xyz_coordinates=origin_dsm_grid_xyz,
            masked_dsm_grid_xyz_coordinates=masked_dsm_grid_xyz,
            masked_dsm_grid_rgb_image=masked_dsm_grid_rgb,
            debug_dir=debug_dir,
        ...
        if self._expand_rate != 1:
            ...
            heat_dsm_grid_rgb = np.array(
                Image.fromarray(masked_dsm_grid_rgb).resize(expanded_size),
                dtype=np.uint8,
            )
        ...
        square_heat_dsm_grid_rgb[top:top_end, left:left_end] = heat_dsm_grid_rgb[
        ...
        return square_heat_dsm_grid_rgb, square_heat_depth_image, roof_layer_info
