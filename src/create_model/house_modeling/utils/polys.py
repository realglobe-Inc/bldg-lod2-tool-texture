...
import numpy as np
from shapely import Polygon, GeometryCollection, Point as GeoPoint
...
def find_final_parent(
    polygon_id: int, longest_neighbor_map: dict[int, tuple[int, float]]
):
    """
...
def merge_small_polys_into_large_polys(
    ...
    for (i, poly_a), (j, poly_b) in itertools.combinations(enumerate(origin_polys), 2):
        ...
        if (
            i not in longest_neighbor_map
            or longest_neighbor_map[i][1] < share_edge_length_sum
        ):
            longest_neighbor_map[i] = (j, share_edge_length_sum)
        if (
            j not in longest_neighbor_map
            or longest_neighbor_map[j][1] < share_edge_length_sum
        ):
            longest_neighbor_map[j] = (i, share_edge_length_sum)

    small_polygon_ids = [
        polygon_id
        for polygon_id, origin_poly in enumerate(origin_polys)
        if origin_poly.area < small_threshold_area
    ]
    last_parent_polygon_id_small_polygon_ids_pair: dict[int, list[int]] = defaultdict(
        list
    )

    for small_polygon_id in small_polygon_ids:
        last_polygon_parent_id = find_final_parent(
            small_polygon_id, longest_neighbor_map
        )
        last_parent_polygon_id_small_polygon_ids_pair[last_polygon_parent_id].append(
            small_polygon_id
        )
    ...
    for (
        last_parent_polygon_id,
        small_polygon_ids,
    ) in last_parent_polygon_id_small_polygon_ids_pair.items():
        group_polygon_ids = [last_parent_polygon_id, *small_polygon_ids]
        target_polys = [
            Polygon(polygon_ijs_list[polygon_id]) for polygon_id in group_polygon_ids
        ]
    ...
    not_merged_polygon_ijs_list = [
        ...
        for polygon_id in range(len(polygon_ijs_list))
        if polygon_id not in merged_polygon_ids
    ]
...
def validate_polygon_ijs_list(polygon_ijs_list: list[list[tuple[float, float]]]):
    ...
    for edge, count in edge_polygon_ids_pair.items():
        assert (
            len(count) <= 2
        ), f"三つ以上のポリゴンが一つのエッジ({edge[0]}, {edge[1]})を持つことは不可能です"

    # 被っているポリゴン検査
    has_polys_collision = False
    indexed_polys = list(enumerate(polys))
    for (index_a, poly_a), (index_b, poly_b) in itertools.combinations(
        indexed_polys, 2
    ):
    ...
    single_edge_count = sum(
        [1 for polygon_ids in edge_polygon_ids_pair.values() if len(polygon_ids) == 1]
    )
    assert single_edge_count == len(
        outer_polygon_ijs
    ), "単一エッジの数と外周ポリゴンのエッジの数が一致しません"


def ensure_counter_clockwise(
    polygon_xyz: list[Union[tuple[float, float, float], tuple[float, float]]],
):
    """
    ポリゴンの頂点が反時計回りになるようにする

    Args:
      polygon_xyz (list[tuple[float, float]]): ポリゴンの頂点座標リスト

    Returns:
      list[int]: 反時計回りにしたポリゴン
    """
    polygon_xys = np.array(polygon_xyz)[:, :2]
    poly = Polygon(polygon_xys)
    if not poly.exterior.is_ccw:
        # 時計回りなら反転させる
        return polygon_xyz[::-1]
