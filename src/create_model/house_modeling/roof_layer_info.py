...
from src.create_model.house_modeling.utils.polys import (
    get_grid_point_ijs,
...
class RoofLayerInfo:
    ...
    @property
    def masked_dsm_grid_rgb_image(self):
        """
        DSM点群のRGB画像
        ...
        return self._masked_dsm_grid_rgb_image

    @property
    def origin_dsm_grid_xyz_coordinates(self):
        """
        ...
        return self._origin_dsm_grid_xyz_coordinates
    ...
    def __init__(
        self,
        origin_dsm_grid_xyz_coordinates: npt.NDArray[np.float_],
        masked_dsm_grid_xyz_coordinates: npt.NDArray[np.float_],
        masked_dsm_grid_rgb_image: npt.NDArray[np.int_],
        debug_dir: str,
        ...
        Args:
          origin_dsm_grid_xyz_coordinates (npt.NDArray[np.uint8]): DSM点群のRGB画像(i,j)のxyz座標(建築物領域以外も高さあり : 3D高さ検出用)
          masked_dsm_grid_xyz_coordinates (npt.NDArray[np.uint8]): DSM点群のRGB画像(i,j)のxyz座標(建築物領域以外は高さ0 : 壁検出用)
          masked_dsm_grid_rgb_image (npt.NDArray[np.int_]): DSM点群のRGB画像(建築物領域以外は白色)
          debug_dir (str): 記録するファイル名
          debug_mode (bool): デバッグモード
        """

        self._origin_dsm_grid_xyz_coordinates = origin_dsm_grid_xyz_coordinates.copy()
        self._masked_dsm_grid_xyz_coordinates = masked_dsm_grid_xyz_coordinates.copy()
        self._masked_dsm_grid_rgb_image = masked_dsm_grid_rgb_image.copy()
        self._debug_mode = debug_mode
        self._debug_dir = debug_dir
        self._height, self._width = masked_dsm_grid_xyz_coordinates.shape[:2]
        self._layer_class = np.full(
            (self._height, self._width), RoofLayerInfo.NO_POINT, dtype=np.int_
        )
        self._layer_class_length = 0

        self._color_palette = self.get_color_palette(
            RoofLayerInfo.RESERVED_COLOR.values()
        )
    ...
    def ij_to_z(self, i: Union[float, int], j: Union[float, int]):
        ...
        height, width = self._masked_dsm_grid_xyz_coordinates.shape[:2]
        i_clipped = min(max(int(round(i)), 0), height - 1)
        j_clipped = min(max(int(round(j)), 0), width - 1)
        return float(self._masked_dsm_grid_xyz_coordinates[i_clipped, j_clipped, 2])
    ...
    def _get_wall_point_positions(self):
        ...
        for i, masked_dsm_grid_xyz_coordinates_j in enumerate(
            self._masked_dsm_grid_xyz_coordinates
        ):
            for j, (x, y, z1) in enumerate(masked_dsm_grid_xyz_coordinates_j):
                ...
                for di, dj in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                    ...
                    if 0 <= i2 < height and 0 <= j2 < width:
                        z2s.append(self._masked_dsm_grid_xyz_coordinates[i2, j2, 2])
    ...
    def _get_xy_ij_pair(self):
        ...
        for i, masked_dsm_grid_xyz_coordinates_j in enumerate(
            ...
            for j, (x, y, _) in enumerate(masked_dsm_grid_xyz_coordinates_j):
                xy_ij[(x, y)] = (i, j)

        return xy_ij

    @staticmethod
    def _is_wall_index(z1: float, z2s: list[float]):
        """
        壁点を判定する
    ...
    def _bfs_layer_fill(self, start_i: int, start_j: int):
        ...
        while queue:
            ...
            # 現在の点の z 座標
            current_z = self._masked_dsm_grid_xyz_coordinates[i, j, 2]
            ...
            for di, dj in directions:
                ...
                if 0 <= i2 < self._height and 0 <= j2 < self._width:
                    if self._layer_class[i2, j2] == RoofLayerInfo.NO_POINT:
                        neighbor_z = self._masked_dsm_grid_xyz_coordinates[i2, j2, 2]

                        # z 座標の差が RoofLayerInfo.WALL_HEIGHT_THRESHOLD 以下なら、同じレイヤーと見なす
                        if (
                            abs(current_z - neighbor_z)
                            <= RoofLayerInfo.WALL_HEIGHT_THRESHOLD
                        ):
    ...
    def _save_image_origin(self):
        ...
        """
        image_origin = Image.fromarray(self._masked_dsm_grid_rgb_image, "RGB")
    ...
    def _save_image_wall_line(
        ...
        """
        masked_dsm_grid_rgb_image_wall_line = self._masked_dsm_grid_rgb_image.copy()
        for i, j in wall_point_positions:
            masked_dsm_grid_rgb_image_wall_line[i, j] = [255, 0, 0]

        image_wall_line = Image.fromarray(masked_dsm_grid_rgb_image_wall_line, "RGB")
        image_wall_line_path = os.path.join(self._debug_dir, file_name)
        image_wall_line.save(image_wall_line_path)

    def save_layer_image(
        self, layer_class: npt.NDArray[np.int_], file_name: str = "layer.png"
    ):
        """
    ...
    def save_roof_line_image(
        self,
        masked_dsm_grid_rgb_image: npt.NDArray[np.uint8],
        ...
        Args:
          masked_dsm_grid_rgb_image (npt.NDArray[np.uint8]): DSM点群のRGB画像
          roof_lines (set[tuple[tuple[int, int], tuple[int, int]]]): 線のリスト
          file_name (str): 記録するファイル名
        """

        # RGB画像データをBGRに変換（OpenCVはBGRを使用）
        image_roof_line = cv2.cvtColor(
            masked_dsm_grid_rgb_image.copy(), cv2.COLOR_RGB2BGR
        )
    ...
    @staticmethod
    def get_color_palette(reserved_colors: list[list[int]]):
        """屋根レイヤーの色の定義する
        ...
        for color_variation_num in [255, 128, 64, 32, 16, 8]:
            ...
            color_palette = list(
                itertools.product(base_colors, repeat=3)
            )  # すべての組み合わせを生成
        ...
        color_palette_result = [
            list(color_palette)
            for color_palette in color_palette_all
    ...
    def get_layer_number_layer_area_polygon_ijs_list_pair(self):
        ...
        layer_number_layer_area_polygon_ijs_list_pair: dict[
            int, list[list[tuple[int, int]]]
        ] = {}

        height, width = self._layer_class.shape

        layer_number_point_ijs_pair: dict[int, list[tuple[int, int]]] = defaultdict(
            list
        )
        ...
        for layer_number in layer_numbers:
            ...
            mask = cv2.inRange(
                image_rgb, np.array(search_color), np.array(search_color)
            )
            ...
            for contour in contours:
                epsilon = epsilon_factor * cv2.arcLength(
                    contour, True
                )  # 輪郭の長さに基づいてepsilonを計算
                approx = cv2.approxPolyDP(contour, epsilon, True)  # 輪郭を単純化
                merged_outline_polygon_ijs = [
                    (point[0][1], point[0][0]) for point in approx
                ]
            ...
            current_merged_polygon_ijs_list = get_polygon_ijs_list(
                [merged_polygons_area]
            )

            last_merged_polygon_ijs_union = unary_union(
                [Polygon(p) for p in last_merged_polygon_ijs]
            )
            current_merged_polygon_ijs_list_union = unary_union(
                [Polygon(p) for p in current_merged_polygon_ijs_list]
            )

            # layer_numberのポリゴンリスト = 今回までマージしたポリゴンリスト - 前回までマージしたのポリゴンリスト
            difference = current_merged_polygon_ijs_list_union.difference(
                last_merged_polygon_ijs_union
            )
            current_polygon_ijs_list = get_polygon_ijs_list([difference])

            layer_number_layer_area_polygon_ijs_list_pair[layer_number] = (
                current_polygon_ijs_list
            )
            ...
            # 輪郭線を描画 (塗りつぶさない)
            if self._debug_mode:
                outline_part_image_rgb = np.full(
                ...
                outline_part_image_rgb = cv2.cvtColor(
                    outline_part_image_rgb, cv2.COLOR_BGR2RGB
                )
                outline_part_image_path = os.path.join(
                    self._debug_dir, f"layer_area_{layer_number}.png"
                )
                cv2.imwrite(outline_part_image_path, outline_part_image_rgb)

        if self._debug_mode:
            outline_all_image_rgb = cv2.cvtColor(
                outline_all_image_rgb, cv2.COLOR_BGR2RGB
            )
    ...
    def add_balcony_layers(
        self, balcony_polygon_ijs_list: list[list[tuple[float, float]]]
    ):
        """
        ...
        balcony_point_ijz_set: set[tuple[int, int, float]] = set()
        for balcony_polygon_ijs in balcony_polygon_ijs_list:
            ...
            for i, j in grid_point_ijs:
                ...
                balcony_point_ijz_set.add((i_clipped, j_clipped, z))
                self._layer_class[i_clipped, j_clipped] = self._layer_class_length

            self._layer_class_length += 1

        if self._debug_mode and balcony_point_ijz_set:
            # バルコニー領域を画像ファイルに保存
            height, width = self._layer_class.shape
            ...
            max_z = np.max([z for _, _, z in balcony_point_ijz_set])
            min_z = np.min([z for _, _, z in balcony_point_ijz_set])

            # デバッグ用の色変更
            for i, j, z in balcony_point_ijz_set:
