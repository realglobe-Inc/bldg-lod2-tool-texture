...
from src.create_model.building_classification.classifier import BuildingClass
...
class ModelingParam:
    ...
    def __init__(self, debug_mode=False) -> None:
        ...
        # GraphCut
        self._graph_cut_height_offset = 4.0  # 地面高さのoffset値
        # self._graph_cut_smooth_weight = 25.0 # 平滑化の重み
        self._graph_cut_smooth_weight = 20.0  # 平滑化の重み
        self._graph_cut_invalid_point_dist = 100.0  # 無効点とする距離閾値
        self._graph_cut_height_diff_th = 2.0  # 高さ差分閾値
        self._graph_cut_dbscan_radius = 0.5  # dbscanの探索半径
        self._graph_cut_dbscan_point_th = 5  # dbscanのコア点判定用の点数閾値
        ...
        self._mbr_max_hierarchies = 100  # 最大階層数
        ...
        self._building_class_cache_path = join(
            dir_name, "data", "building_class_cache.json"
        )

        # ビルの形状をキャッシュ化
        self.building_class_cache: dict[str, BuildingClass] = {}
        if os.path.exists(self._building_class_cache_path) and self._debug_mode:
            with open(self._building_class_cache_path, "r") as f:
                self.building_class_cache = json.load(f)
    ...
    @property
    def graph_cut_height_offset(self) -> float:
        """GraphCutの地面高さのoffset値

        Returns:
            float: GraphCutの地面高さのoffset値
        """
        return self._graph_cut_height_offset

    @graph_cut_height_offset.setter
    def graph_cut_height_offset(self, value: float):
        """GraphCutの地面高さのoffset値

        Args:
            value (float): GraphCutの地面高さのoffset値
        """
        self._graph_cut_height_offset = value

    @property
    def graph_cut_smooth_weight(self) -> float:
        """GraphCutの平滑化の重み

        Returns:
            float: GraphCutの平滑化の重み
        """
        return self._graph_cut_smooth_weight

    @graph_cut_smooth_weight.setter
    def graph_cut_smooth_weight(self, value: float):
        """GraphCutの平滑化の重み

        Args:
            value (float): GraphCutの平滑化の重み
        """
        self._graph_cut_smooth_weight = value

    @property
    def graph_cut_invalid_point_dist(self) -> float:
        """GraphCutの無効点とする距離閾値

        Returns:
            float: GraphCutの無効点とする距離閾値
        """
        return self._graph_cut_invalid_point_dist

    @graph_cut_invalid_point_dist.setter
    def graph_cut_invalid_point_dist(self, value: float):
        """GraphCutの無効点とする距離閾値

        Args:
            value (float): GraphCutの無効点とする距離閾値
        """
        self._graph_cut_invalid_point_dist = value

    @property
    def graph_cut_height_diff_th(self) -> float:
        """GraphCutの高さ差分閾値

        Returns:
            float: GraphCutの高さ差分閾値
        """
        return self._graph_cut_height_diff_th

    @graph_cut_height_diff_th.setter
    def graph_cut_height_diff_th(self, value: float):
        """GraphCutの高さ差分閾値

        Args:
            value (float): GraphCutの高さ差分閾値
        """
        self._graph_cut_height_diff_th = value

    @property
    def graph_cut_dbscan_radius(self) -> float:
        """GraphCut処理でのdbscanの探索半径

        Returns:
            float: GraphCut処理でのdbscanの探索半径
        """
        return self._graph_cut_dbscan_radius

    @graph_cut_dbscan_radius.setter
    def graph_cut_dbscan_radius(self, value: float):
        """GraphCut処理でのdbscanの探索半径

        Args:
            value (float): GraphCut処理でのdbscanの探索半径
        """
        self._graph_cut_dbscan_radius = value

    @property
    def graph_cut_dbscan_point_th(self) -> int:
        """GraphCut処理でのdbscanのコア点判定用の点数閾値

        Returns:
            int: GraphCut処理でのdbscanのコア点判定用の点数閾値
        """
        return self._graph_cut_dbscan_point_th

    @graph_cut_dbscan_point_th.setter
    def graph_cut_dbscan_point_th(self, value: int):
        """GraphCut処理でのdbscanのコア点判定用の点数閾値

        Args:
            value (int): GraphCut処理でのdbscanのコア点判定用の点数閾値
        """
        self._graph_cut_dbscan_point_th = value
    ...
    @property
    def mbr_max_hierarchies(self) -> float:
        """MBRの最大階層数

        Returns:
            float: MBRの最大階層数
        """
        return self._mbr_max_hierarchies

    @mbr_max_hierarchies.setter
    def mbr_max_hierarchies(self, value: float):
        """MBRの最大階層数

        Args:
            value (float): MBRの最大階層数
        """
        self._mbr_max_hierarchies = value
    ...
    @property
    def building_class_cache_path(self) -> str:
        """建物分類の推論のキャッシュファイルパス
        Returns:
            str: 建物分類の推論のキャッシュファイルパス
        """
        return self._building_class_cache_path
