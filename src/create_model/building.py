...
from .building_classification.classifier import BuildingClass
from .building_classification.classify_building import classify_building
from .building_modeling.create_model import BuildingModelBuilder
from .create_model_exception import ModelingException
from .house_modeling.create_model import HouseModelBuilder
from .las_manager import LasManager, PointCloud
    ...
    def create(self, las_swap_xy=False, debug_mode=False) -> None:
        ...
        if os.path.exists(cache_file_path) and debug_mode is True:
            with open(cache_file_path, "rb") as f:
                ...
                cloud, min_ground_height, graph_cut_height, thin_rate = cached_dsm
        else:
            # 点群データの取得
            ...
            # 建物点群の取得
            cloud, min_ground_height, graph_cut_height, thin_rate = las_mng.get_points(
            ...
            if debug_mode:
                with open(cache_file_path, "wb") as f:
                    pickle.dump(
                        (cloud, min_ground_height, graph_cut_height, thin_rate), f
                    )
        ...
        if building_class == BuildingClass.FLAT:
            ...
            BuildingModelBuilder(
                cloud=cloud,
                shape=self._shape,
                graph_cut_height=graph_cut_height,
                grid_size=grid_size,
                building_id=self._id,
