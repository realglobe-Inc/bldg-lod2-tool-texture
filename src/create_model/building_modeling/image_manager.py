...
class TFWReadException(ModelingException):
    """TFW読み込みエラークラス"""
...
class TIFFReadException(ModelingException):
    """TIFF読み込みエラークラス"""
...
class ImageInfo:
    ...
    def _read_image(
        self, file_path: str, flags=cv.IMREAD_COLOR, dtype=np.uint8, keep=False
    ):
        """画像読み込み

    Args:
        file_path (str): 画像ファイルパス
        flags (cv2.ImreadModes, optional): \
            画像読み込みモード. Defaults to cv2.IMREAD_COLOR.
...
class ImageManager:
    ...
    def read_files(self, folder_path: str):
        ...
        if not os.path.isdir(folder_path):
            # フォルダが存在しない場合
            msg = "{}.{}, {}".format(
                class_name, func_name, ModelingMessage.ERR_MSG_IMG_MNG_FOLDER_NOT_FOUND
            )
        ...
        if len(files) == 0:
            # tiffファイルが存在しない場合
            msg = "{}.{}, {}".format(
                class_name, func_name, ModelingMessage.ERR_MSG_IMG_MNG_IMAGE_NOT_FOUND
            )
            raise ModelingException(msg)
        ...
        for file in files:
            try:
                ...
                # TFWの読み込みに失敗した場合
                msg = "{}.{}, {}".format(
                    class_name, func_name, ModelingMessage.ERR_MSG_IMG_MNG_TFW_READ
                )
                ...
                # TIFF読み込みに失敗した場合
                msg = "{}.{}, {}".format(
                    class_name, func_name, ModelingMessage.ERR_MSG_IMG_MNG_TIFF_READ
                )
                Log.output_log_write(
                    LogLevel.WARN, ModuleType.MODEL_ELEMENT_GENERATION, msg
                )
        ...
        if len(self._images) == 0:
            # 全TIFFファイルの読み込みに失敗
            msg = "{}.{}, {}".format(
                class_name, func_name, ModelingMessage.ERR_MSG_IMG_MNG_READ
            )
            raise ModelingException(msg)
        ...
        if len(self._images) != len(files):
            ...
            Log.output_log_write(
                ...
                ModelingMessage.WARN_MSG_IMG_MNG_READ,
            )
