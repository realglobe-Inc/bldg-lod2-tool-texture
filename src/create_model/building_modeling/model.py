...
from shapely import Polygon, LineString, LinearRing, MultiPolygon, Point
...
from .plane_info import PlaneInfo
...
class LineInfo:
    ...
    def __init__(self, line: LineString = None, id1=-1, id2=-1) -> None:
        """コンストラクタ

        Args:
            line (LineString, optional): 壁面位置. Defaults to None.
...
class FaceInfo:
    def __init__(self, points: NDArray, type=FaceType.UNKNOWN, label=-1) -> None:
        """面情報
...
class InsertPoint:
    ...
    def __init__(self, x: float, y: float, roof_id: int, index: int, t: float) -> None:
        """コンストラクタ
...
class Model:
    ...
    @shape.setter
    def shape(self, value: Polygon):
        """建物外形ポリゴン

        Args:
            value (Polygon): 建物外形ポリゴン
        """
        self._shape = value

    @property
    def use_hierarchy_classify(self) -> bool:
        """階層分類フラグ
        ...
        """
        return self._use_hierarchy_classify

    @use_hierarchy_classify.setter
    def use_hierarchy_classify(self, value: bool):
        """階層分類フラグ
        ...
        Note:
            ...
            建物中央にある屋上設備等はモデル化しない
        """
        self._use_hierarchy_classify = value

    def __init__(self, id, shape: Polygon, use_hierarchy_classify=False) -> None:
        """コンストラクタ

        Args:
            id (str): 建物ID
            shape (Polygon): 建物外形ポリゴン
            use_hierarchy_classify (bool, optional): 階層分類フラグ. Defaults to False.
        Note:
            use_hierarchy_classify=Trueの場合、建物外形ポリゴンに隣接する屋根を選出し、
            建物中央にある屋上設備等はモデル化しない
        """
        ...
        self._use_hierarchy_classify = use_hierarchy_classify  # 階層分類フラグ
        self._group_list = []  # 親屋根でグルーピングした結果
        # 壁面作成用形状(最外壁と島屋根の外壁)
        self._outlines: list[PolylineInfo] = []

    def _point_to_line_dist(
        self, pos: NDArray, line_pos: NDArray, line_vec: NDArray
    ...
    @staticmethod
    def _check_inclusion_relationships(clusters: list[ClusterInfo]):
        """包含関係の確認
        ...
        for i in np.arange(len(clusters)):
            if clusters[i].roof_polygon is None or GeoUtil.is_zero(
                clusters[i].roof_polygon.area
            ):
                continue
            ...
            for j in np.arange(len(clusters)):
                ...
                if clusters[j].roof_polygon is None or GeoUtil.is_zero(
                    clusters[j].roof_polygon.area
                ):
                    continue
    ...
    def create_model_surface(self, clusters: list[ClusterInfo], ground_height: float):
        """モデル面の作成
        ...
        # 頂点マージ
        self._merge_vertex(
            xy_dist=param.model_point_merge_xy_dist, reso=param.model_point_merge_z_reso
        )
    ...
    def _create_roof_surface(self, clusters: list[ClusterInfo]) -> list:
        ...
        for cluster in clusters:
            if cluster.roof_polygon is None or GeoUtil.is_zero(
                cluster.roof_polygon.area
            ):
                continue

            # 屋根の高さ
            if not self._use_hierarchy_classify:
                # 階層分類を行わない場合
                cluster.roof_height = cluster.points.max[2]

            if len(cluster.children) == 0:
                # 内包する屋根がない場合
                roof: Polygon = cluster.roof_polygon
                ...
                face = FaceInfo(points=points, type=FaceType.ROOF, label=cluster.id)
                surface.append(face)
                ...
                for i in cluster.children:
                    # 穴の設定
                    child_poly: Polygon = clusters[i].roof_polygon
                ...
                while num != len(parts):
                    ...
                    for polygon in parts:
                        if len(polygon.interiors) > 0:
                            # 穴がある場合
                            # 穴の中心座標を取得
                            hole: LinearRing = polygon.interiors[0]
                            center = np.array([hole.centroid.x, hole.centroid.y])
                            ...
                            for i in np.arange(bbox_num - 1):
                                pos1 = np.array([coords[i][0], coords[i][1]])
                                pos2 = np.array([coords[i + 1][0], coords[i + 1][1]])
                            ...
                            while current != start:
                                ...
                                pos2 = np.array([coords[next][0], coords[next][1]])

                                vec = pos2 - pos1
                                ret, h, online1, online2 = GeoUtil.cross_point(
                                    vec1=vec, pos1=pos1, vec2=line, pos2=start_pos
                                )
                            ...
                            start_indices = np.array([start, end])
                            end_indices = np.array([end, start])
                            for i in np.arange(len(start_indices)):
                                points = start_points[i]
                                current = start_indices[i]
                                while current != end_indices[i]:
                                    pos = np.array(
                                        [coords[current][0], coords[current][1]]
                                    )
                                ...
                                points = np.vstack((points, start_points[i]))
                                ...
                                # ポリゴンのみを抽出
                                geo_list = GeoUtil.separate_geometry(and_region)
                                select = [
                                    poly
    ...
    def _surface_preparation(
        self, clusters: list[ClusterInfo], angle_th=3.0, sampling_step=0.01, dist_th=0.1
    ) -> None:
        """面作成準備
    ...
    """
        ...
        # 辺の角度でグルーピング
        angle_clusters = (
            self._angle_clustering(rotate_edges, angle_th=angle_th)
            if len(rotate_edges) > 0
            else []
        )
        ...
        for i, cluster in enumerate(edge_clusters):
            ...
            sampling_points = self._sampling_edge(
                cluster=cluster, points=rotate_points, sampling_step=sampling_step
            )
        ...
        for insert_pt in insert_pt_list:
            ...
            clusters[insert_pt.roof_id].roof_line = np.insert(
                clusters[insert_pt.roof_id].roof_line, insert_pt.index, pt, axis=0
            )
            ...
            if base_idx == self._NO_PARENT_IDX:
                ...
                if isinstance(base_polygon, MultiPolygon):
                    ...
                    for poly in base_polygon.geoms:
                        ...
                        if area > max_area:
                            max_area = area
                            self._shape = Polygon(poly.exterior)
                else:
                    self._shape = Polygon(base_polygon.exterior)
    ...
    def _grouping_roof(self, clusters: list[ClusterInfo]) -> None:
        ...
        roots = [
            ...
            if (
                cluster.roof_polygon is not None
    ...
    def _calc_rotate_angle(self) -> Tuple[float, NDArray]:
        ...
        vectors = [
            points_list[i + 1] - points_list[i] for i in range(len(points_list) - 1)
        ]
    ...
    @staticmethod
    def _rotate_edge(
        clusters: list[ClusterInfo], rot: Rotation, pt: NDArray
    ) -> Tuple[NDArray, dict]:
        """回転後の辺ベクトルと座標点の作成
        ...
        for i, cluster in enumerate(clusters):
            if cluster.roof_line is not None:
                ...
                # 辺ベクトル
                vectors = [
                    [(points[n + 1] - points[n])[0], (points[n + 1] - points[n])[1]]
    ...
    @staticmethod
    def _sampling_edge(cluster: NDArray, points: dict, sampling_step=0.1) -> list:
    ...
    def _search_for_missing_vertices(
        ...
        rev_rot: Rotation,
        base_pt: NDArray,
        insert_pt_list: list[InsertPoint],
        dist_th=0.1,
    ):
        """不足頂点の確認
        ...
        def _check_point(
            sampling_points: NDArray, pt: NDArray, dist_th=0.1
        ) -> Tuple[NDArray, float]:
            """途中点確認
            ...
            dists, indices = nn.kneighbors(pt.reshape(1, 2), return_distance=True)
            dists = dists[:, 0]
            indices = indices[:, 0]

            if dists[0] < dist_th:
                vec1 = sampling_points[-1] - sampling_points[0]
                vec2 = sampling_points[indices[0]] - sampling_points[0]
                ...
                dst_pt = np.array(
                    [
                        sampling_points[indices[0]][0],
                        sampling_points[indices[0]][1],
                        0.0,
                    ]
                )
                return dst_pt, t
        ...
        for m in range(len(edges) - 1):
            for n in range(m + 1, len(edges)):
                ...
                param_list = [[n, start_m], [n, end_m], [m, start_n], [m, end_n]]
                ...
                for index, input_pt in param_list:
                    ...
                    if data is not None:
                        ...
                        exist = filter(
                            ...
                            and x.index == item.index
                            and GeoUtil.is_same_value(x.t, item.t),
                            insert_pt_list,
                        )
                        if len(list(exist)) == 0:
    ...
    def _create_wall_surface(
        self, clusters: list[ClusterInfo], ground_height: float
    ) -> list[FaceInfo]:
        """壁面作成処理
        ...
        for line_info in line_infos:
            ...
            if line_info.id2 < 0:
                ...
                # 島屋根(穴)の確認
                if (
                    self._NO_PARENT_IDX < cluster1.parent == line_info.id2
    ...
    @staticmethod
    def _check_reverse_wall(wall_points: NDArray, normal: NDArray) -> bool:
        """壁面の頂点列の反転確認
    ...
    @staticmethod
    def _create_ground_surface(shape: Polygon, ground_height: float) -> FaceInfo:
        """地面作成
    ...
    def _solid(self, dist_th=0.001):
        ...
        for i in np.arange(len(self._faces)):
            for j in np.arange(len(self._faces)):
                ...
                if (
                    face1.face_type == FaceType.ROOF
                    and face2.face_type == FaceType.GROUND
                ) or (
                    face1.face_type == FaceType.GROUND
                ...
                new_points = self._add_missing_vertex(
                    target=face1.points, candidates=face2.points, dist_th=dist_th
                )

                # 更新
                self._faces[i].points = new_points

    def _add_missing_vertex(self, target: NDArray, candidates: NDArray, dist_th=0.001):
        """不足頂点の追加処理
        ...
        for i in np.arange(len(candidates)):
            ...
            for j in np.arange(len(tmp_points)):
                ...
                if 0.0 < t < 1.0 and dist < min_dist:
                    index = next
                    min_dist = dist

            if index > -1:
                # 頂点追加
                tmp_points = np.insert(tmp_points, index, candidates[i], axis=0)
    ...
    def _euclidean_clustering(
        self, cloud: NDArray, num_th: int, dist_th=0.9, ignore_z=False
    ):
        """ユークリッド距離によるクラスタリング
        ...
        if num_th < 1:
            msg = "{}.{}, {}".format(
                class_name, func_name, ModelingMessage.ERR_MSG_MODEL_NUM_TH
            )
            raise ModelingException(msg)

        if dist_th < 0:
            msg = "{}.{}, {}".format(
                class_name, func_name, ModelingMessage.ERR_MSG_MODEL_DIST_TH
            )
            raise ModelingException(msg)
        ...
        for n in np.arange(0, len(cloud)):
            ...
            while i < len(cluster_index):
                ...
                indexes = tree.query_radius(tmp_cloud[target : target + 1], r=dist_th)
                indexes = indexes[0]
    ...
    @staticmethod
    def _angle_clustering(edges: NDArray, angle_th=5.0) -> list[NDArray]:
        """辺の角度によるクラスタリング
        ...
        # 角度算出
        vec_x = np.array((1.0, 0.0))
        tmp_angles = [GeoUtil.signed_angle(vec_x, vec) for vec in tmp_edges]
        angles = [
            [angle + 180.0, 0] if angle < 0 else [angle, 0] for angle in tmp_angles
        ]

        angles = np.array(angles)
        db = DBSCAN(eps=angle_th, min_samples=1)
        db.fit(angles)
        labels = db.labels_
        n_clusters = len(set(labels)) - (1 if -1 in labels else 0)
        for ci in range(n_clusters):
            indices = np.where(labels == ci)[0]
            new_cluster = edges[indices]
    ...
    def _edge_dist_clustering(
        edges: NDArray, points: dict, dist_th=0.1, sampling_step=0.01
    ) -> list[NDArray]:
        """辺の距離によるクラスタリング
        ...
        db = DBSCAN(eps=dist_th, min_samples=1)
        db.fit(sampling_data)
        labels = db.labels_
        n_clusters = len(set(labels)) - (1 if -1 in labels else 0)
        for ci in range(n_clusters):
            indices = np.where(labels == ci)[0]
            edge_ids = np.unique(sampling_id[indices])
    ...
    @staticmethod
    def _merge_vertex_2d(clusters: list[ClusterInfo], dist=0.1) -> None:
        """屋根面の頂点のマージ
        ...
        indexes = [
            i for i, cluster in enumerate(clusters) if cluster._roof_line is not None
        ]
        ...
        db = DBSCAN(eps=dist, min_samples=1)
        db.fit(tmp_points[:, 0:2])
        labels = db.labels_
        n_clusters = len(set(labels)) - (1 if -1 in labels else 0)
        for ci in range(n_clusters):
            indices = np.where(labels == ci)[0]
            result = tmp_points[indices]
