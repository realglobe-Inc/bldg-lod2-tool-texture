...
from shapely import GeometryCollection, MultiLineString, MultiPolygon, MultiPoint
...
class GeoUtil:
    ...
    @classmethod
    def is_zero(cls, value: float) -> bool:
        """浮動小数点が0であるかチェック
    ...
    @staticmethod
    def point_to_line_dist(pos: NDArray, line_pos: NDArray, line_vec: NDArray):
        """点と直線との距離
    ...
    """
        ...
        len_sqv = GeoUtil.size(line_vec)
        len_sqv *= len_sqv
        t = 0.0

        if len_sqv > 0.0:
            t = np.dot(line_vec, pos - line_pos) / len_sqv
    ...
    @staticmethod
    def cross_point(vec1: NDArray, pos1: NDArray, vec2: NDArray, pos2: NDArray):
        """2直線の交点
        ...
        is2d = len(pos1) == 2 and len(vec1) == 2 and len(pos2) == 2 and len(vec2) == 2
        if not is2d:
            # 入力データが2次元座標ではない
    ...
    def separate_geometry(geom) -> list:
        ...
        if geom is not None:
            if isinstance(geom, GeometryCollection):
                ...
                isinstance(geom, MultiLineString)
