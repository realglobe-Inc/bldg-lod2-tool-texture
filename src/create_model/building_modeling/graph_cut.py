...
class GraphCut:
    ...
    @staticmethod
    def _get_distance_to_plane(
        points: NDArray, offset: NDArray, alpha: float, beta: float, gamma: float
    ):
        """各点と平面との距離
    ...
    @staticmethod
    def _get_bbox(xy_points: NDArray):
        """バウンディングボックスの作成
    ...
    def _check_ground_cluster(
        clusters: list[ClusterInfo], ground_height: float
    ) -> list[bool]:
        """地面の確認
        ...
        for i, cluster in enumerate(clusters):
            ...
            if cluster_height < ground_height:
                ...
                if (
                    is_contained
                    or candidate_area[i] < candidate_area_th
        ...
        roofs = [
            i
            for i in range(len(candidate_is_ground))
    ...
    def _separate_points(
        self, src_points: NDArray, clusters: list[ClusterInfo], ground_height: float
        ...
        other_points_ind = np.setdiff1d(building_points_ind, cluster_points_ind)
    ...
    Raises:
        ModelingException: 点群に対応するオルソ画像がない場合
    ...
    """
        ...
        roof_cloud, ground_cloud, ground_flags = self._separate_points(
            src_points=src_points, clusters=src_clusters, ground_height=ground_height
        )

        if len(roof_cloud.get_points()) == 0:
            # 屋根点が取得でき無かった場合
            msg = "{}.{}, {}".format(
                class_name, func_name, ModelingMessage.ERR_MSG_GRAPH_CUT_NO_ROOF_POINTS
            )
        ...
        xy_points = np.round(xy_points).astype(np.int64)
        ...
        for i in range(n_labels):
            d[ys, xs, i] = dists[i]
        d[ys, xs, n_labels] = np.ones(len(roof_cloud.get_points())) * invalid_point_dist
        ...
        for merged_labels_ in merged_labels:
            ...
            for i in merged_labels_:
                indices = roof_labels == i
                if np.sum(indices) == 0:
                    continue
                merged_points_xyz += roof_cloud.get_points()[indices].tolist()
                merged_points_label += [i] * len(roof_cloud.get_points()[indices])
            ...
            db.fit(merged_points_xyz[:, 0:2])
            db_labels = db.labels_
            for i in range(db_labels.max() + 1):
                indices = db_labels == i
                new_roof_points += merged_points_xyz[indices].tolist()
                new_roof_labels += [cur_label] * len(merged_points_xyz[indices])
                new_label_heights.append(
                    np.mean(label_heights[np.unique(merged_points_label[indices])])
                )
                cur_label += 1
        ...
        if len(new_roof_points) == 0:
            ...
            msg = "{}.{}, {}".format(
                class_name,
                func_name,
                ModelingMessage.ERR_MSG_GRAPH_CUT_NO_MERGED_ROOF_POINTS,
            )
            raise ModelingException(msg)
        ...
        if len(ground_cloud.get_points()) > 0:
            ...
            indices = nn.kneighbors(
                ground_cloud.get_points()[:, 0:2], return_distance=False
            )[:, 0]
            ground_labels = roof_labels[indices]
        ...
        gc_clusters = []
        for i in range(n_labels):
            indices = building_labels == i
            points = building_points[indices]
            if len(points) == 0:
                continue
