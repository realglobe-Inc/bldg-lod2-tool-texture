...
class Log(Singleton):
    ...
    def log_footer(self):
        ...
        # 実行ログファイル出力
        Log._main_log_file[Log.FORMAT_OFF].info(f"\nEnd Time : {end_time}")
        Log._main_log_file[Log.FORMAT_OFF].info(f"Process Time: {process_time}")

        # 標準出力
        Log._standard_log[Log.FORMAT_OFF].info(f"\nEnd Time : {end_time}")
        Log._standard_log[Log.FORMAT_OFF].info(f"Process Time: {process_time}")
    ...
    def output_summary(self, buildings):
        ...
        # モジュールログファイルパス作成
        file_path = os.path.join(Log.output_log_folder_path, "model_create_result.csv")
