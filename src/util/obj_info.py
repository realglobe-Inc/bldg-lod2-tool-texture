...
class BldElement:
    ...
    @classmethod
    def get_element_type(cls, str) -> BldElementType:
        if str in cls._element_dict:
            return cls._element_dict[str]
        else:
            return BldElementType.NONE

    @classmethod
    def get_element_str(cls, element_type) -> str:
        r_str = ""
        for key, value in cls._element_dict.items():
...
class ObjInfo:
    ...
    # def set_v_list(self, point_list):
    #     """座標値の設定
    ...
    def read_file(self, file_path: str, err_message=None):
        """OBJファイル入力

        Args:
            file_path (str): OBJファイルパス
        """
    ...
    @staticmethod
    def _get_v(s_list) -> Point:
        """座標値情報読み込み

        座標値記述フォーマット: v x y z

        Args:
            s_list (str[]): 座標値文字列リスト
