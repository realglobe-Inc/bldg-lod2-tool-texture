...
from .coordinate_converter import DsmCoordCityGmlCoordConverter
from .coordinate_converter import DsmCoordCityGmlCoordConverterException
from .log import Log, ModuleType, LogLevel
from .obj_info import BldElementType, ObjInfo
from .param_manager import ParamManager
from .result_type import ProcessResult, ResultType
from ..third_party import plateaupy as plapy
...
class CityGmlManager:
    ...
    class BuildInfo:
        ...
        class Lod2Info:
            ...
            def __init__(self):
                ...
                self.id_base = ""  # idベース
                self.pos_list = []  # 建物形状座標リスト
                self.tex_coord_list = []  # テクスチャ画像座標リスト
        ...
        def __init__(self):
            ...
            self.city_gml_filename = ""

        def append_lod2_info(self, face_type, pos_list, coord_list, id):
            """ポリゴン情報追加

            Args:
                face_type (BldElementType): ポリゴン種別
                pos_list (Point[]): 建物形状座標リスト
                coord_list (Point[]): テクスチャ画像リスト
                id (str): ポリゴンのIDベース
            """
            lod2info = self.Lod2Info()
            lod2info.face_type = face_type
            lod2info.pos_list = pos_list
            lod2info.tex_coord_list = coord_list
            lod2info.id_base = id
    ...
    def __init__(self, param_manager: ParamManager) -> None:
        ...
        self.city_gml_info: list[CityGmlManager.BuildInfo] = []
        self.lod1_file_path = ""
    ...
    def read_file(
        ...
        try:
            ...
            input_file_path = os.path.join(
                self.param_manager.city_gml_folder_path, file_name
            )
            ...
            if os.path.exists(city_gml_cache_file_path) and debug_mode is True:
                with open(city_gml_cache_file_path, "rb") as f:
                    self.city_gml_info = pickle.load(f)

                return restype, self._filter_buildings_by_target_building_ids(
                    self.city_gml_info, target_building_ids
                )

            pl_bldg = plapy.PlBldg(self.lod1_file_path)

            if not pl_bldg.buildings:
            ...
            for bldg in pl_bldg.buildings:
                b_info = self.BuildInfo()
                # 建物ID
                b_info.build_id = bldg.id
                # citygmlファイル名
                b_info.city_gml_filename = file_name

                # 建物外形
                if bldg.lod0_roof_edge:
                    b_info.lod0_pos_list = bldg.lod0_roof_edge[0]
                    b_info.read_lod0_model = ProcessResult.SUCCESS

                elif bldg.lod0_foot_print:
                    b_info.lod0_pos_list = bldg.lod0_foot_print[0]
                    b_info.read_lod0_model = ProcessResult.SUCCESS

                else:
                    # 建物外形が見つからない場合
                    b_info.read_lod0_model = ProcessResult.ERROR
                    not_found_outline_num += 1
                    Log.output_log_write(
                        LogLevel.WARN,
                        ModuleType.INPUT_CITY_GML,
                        f"Outline not found {bldg.id}",
                    )
                    restype = ResultType.WARN

                if target_geo_areas is None:
                    self.city_gml_info.append(b_info)
                else:
                    for target_geo_area in target_geo_areas:
                        ...
                        # 緯度（latitude）の最小値と最大値を取得
                        b_info_lat_min = np.min(b_info.lod0_pos_list[:, 0])
                        b_info_lat_max = np.max(b_info.lod0_pos_list[:, 0])

                        # 経度（longitude）の最小値と最大値を取得
                        b_info_lon_min = np.min(b_info.lod0_pos_list[:, 1])
                        b_info_lon_max = np.max(b_info.lod0_pos_list[:, 1])

                        # 建物の領域と検索領域が被っているか
                        if (
                            target_geo_area_lat_max >= b_info_lat_min
                            and target_geo_area_lat_min <= b_info_lat_max
                            and target_geo_area_lon_max >= b_info_lon_min
                            and target_geo_area_lon_min <= b_info_lon_max
                        ):
                            self.city_gml_info.append(b_info)
                            break

            if len(self.city_gml_info) == not_found_outline_num:
                raise Exception("All outline not found")

            # デバッグ : CityGMLファイル読み込みを早くするため、pickle でキャッシュ化
            if debug_mode:
                with open(city_gml_cache_file_path, "wb") as f:
                    pickle.dump(self.city_gml_info, f)
            ...
            Log.output_log_write(
                LogLevel.MODEL_ERROR, ModuleType.INPUT_CITY_GML, "File not found"
            )
            ...
            Log.output_log_write(LogLevel.MODEL_ERROR, ModuleType.INPUT_CITY_GML, e)
            return ResultType.ERROR, None
    ...
    def write_file(self, file_name: str, image_format: str) -> ResultType:
        ...
        try:
            ...
            # OBJ→CityGMLへ情報コピー
            ret = self._copy_obj_data(Config.OUTPUT_TEX_OBJ_DIR)

            # CityGMLファイル書き出し
            if ret != ResultType.ERROR:
                pl_obj = plapy.PlObj()
                tree, root = pl_obj.load_file(self.lod1_file_path)
                ...
                self._nsmap = pl_obj.remove_none_key_from_dic(root.nsmap)
                ...
                # テクスチャ情報がある場合
                tex_app_mem_elem = lxml.etree.Element(f"{self._app_ns}appearanceMember")
                tex_app_elem = lxml.etree.SubElement(
                    tex_app_mem_elem, f"{self._app_ns}Appearance"
                )
                ...
                for bldg in buildings:
                    ...
                    target_city_gml = None
                    for city_gml in self.city_gml_info:
                        if city_gml.read_lod0_model is ProcessResult.ERROR:
                            # LOD0データの取得に失敗しているものは除外
                            continue

                        if bldg.get(f"{self._gml_ns}id") == city_gml.build_id:
                            target_city_gml = city_gml
                            break
                    if target_city_gml is None:
                        continue
                    ...
                    for (
                        num,
                        bld_elem,
                    ) in enumerate(list(bldg)):
                        if "lod1Solid" in bld_elem.tag:
                            wr_idx = num
                            break

                    # lod2Solidのエレメント作成と追加
                    if len(target_city_gml.lod2_info):
                        lod2_elem = self._create_lod2solid_elem(
                            target_city_gml.lod2_info
                        )
                        wr_idx += 1
                        bldg.insert(wr_idx, lod2_elem)

                        # bldg:boundedByのエレメント作成と追加
                        for info in target_city_gml.lod2_info:
                            bounded_elem = self._create_bounded_by_elem(info)
                            wr_idx += 1
                            bldg.insert(wr_idx, bounded_elem)

                    # app:surfaceDataMemberのエレメント作成
                    if target_city_gml.tex_img_uri is not None:
                        surf_ret.append(
                            self._create_surface_data_elem(
                                tex_app_elem,
                                target_city_gml.tex_img_uri,
                                target_city_gml.lod2_info,
                                image_format,
                            )
                        )
            ...
            in_dir = self.param_manager.city_gml_folder_path
            ...
            # 該当フォルダの検索
            in_tex_dir = f"${base_name}_appearance"
            l_in = [s for s in files_dir if in_tex_dir == s]

            if len(l_in) == 1:
                # 出力後フォルダ名
                # [メッシュコード]_[地物型]_[CRS]_[オプション]_appearance
                out_tex_dir = os.path.join(out_dir, in_tex_dir)

                if not os.path.isdir(out_tex_dir):
                    os.mkdir(out_tex_dir)

                for getfile in glob.glob(os.path.join(in_dir, l_in[0], "*")):
                    filename = os.path.basename(getfile)
                    shutil.copy(getfile, os.path.join(out_tex_dir, filename))

            return ret

        except Exception as e:
            shutil.copy(self.lod1_file_path, self.param_manager.output_folder_path)
            Log.output_log_write(LogLevel.MODEL_ERROR, ModuleType.OUTPUT_CITY_GML, e)
            return False

    def _copy_obj_data(self, obj_dir):
        """OBJファイル情報をCityGML形式にコピー

        Args:
            obj_dir (str): OBJフォルダパス
        ...
        try:
            if not os.path.isdir(obj_dir):
                ...
                raise FileNotFoundError
            num_no_obj = 0
            for num, city_gml in enumerate(self.city_gml_info, 1):
                obj_path = os.path.join(obj_dir, f"{city_gml.build_id}.obj")
                if os.path.exists(obj_path):
                    self._obj_info.read_file(obj_path)
                    # 建物形状座標・テクスチャ座標の取得
                    self._get_obj_data(city_gml)
                    # テクスチャ画像URIの取得
                    mtl_info = self._obj_info.get_mtl_info()
                    if mtl_info is not None and city_gml.build_id in mtl_info:
                        city_gml.tex_img_uri = mtl_info[city_gml.build_id].map_kd
                else:
                    # OBJファイルが見つからない場合
                    Log.output_log_write(
                        LogLevel.WARN,
                        ModuleType.OUTPUT_CITY_GML,
                        f"Obj file not found {obj_path}",
                    )
                    num_no_obj += 1

            if num == num_no_obj:
                # 全てのOBJファイルが見つからない場合
                raise Exception("All obj file not found")
            else:
                return ResultType.SUCCESS

        except FileNotFoundError:
            Log.output_log_write(
                LogLevel.MODEL_ERROR, ModuleType.OUTPUT_CITY_GML, "Folder not found"
            )
    ...
    def _get_obj_data(self, city_gml):
        """ポリゴン種別ごとに建物形状座標・テクスチャ画像座標の取得を行う

        Args:
            city_gml (BuildInfo[]): CityGML建物情報リスト
        """
        num = 0
        for face_type in self._face_type.keys():
            texture_list_w = self._obj_info.get_texture_list(face_type)
            polygon_list_w = self._obj_info.get_polygon_list(face_type)
            if polygon_list_w:
                for poly, tex in zip(polygon_list_w, texture_list_w):
                    id = city_gml.build_id + "_" + str(num)
                    city_gml.append_lod2_info(face_type, poly, tex, id)
                    num += 1
    ...
    def _create_bounded_by_elem(self, info):
        ...
        lat, lon = self._dsm_coord_city_gml_coord_converter.to_polar(
            info.pos_list[0].x, info.pos_list[0].y
        )
        str_list.append(str(lat))
        str_list.append(str(lon))
        str_list.append(str(info.pos_list[0].z))

        elem9.text = " ".join(str_list)

        return elem1

    def _create_surface_data_elem(self, app_elem, uri, lod2_info, image_format):
        ...
        for info in lod2_info:
            str_list = []
            for coord_list in info.tex_coord_list:
                if coord_list is not None:
                    str_list.append(str(coord_list.x))
                    str_list.append(str(coord_list.y))
            # 先頭座標を末尾に追加する
            if info.tex_coord_list[0] is not None:
                str_list.append(str(info.tex_coord_list[0].x))
                str_list.append(str(info.tex_coord_list[0].y))
