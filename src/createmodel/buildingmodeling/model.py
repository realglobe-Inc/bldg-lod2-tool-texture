...
class Model:
    ...
    def __init__(self, id, shape: geo.Polygon, use_hier_classify=False) -> None:
        """コンストラクタ
    ...
    def _rotate_edge(
        self, clusters: list[ClusterInfo], rot: Rotation, pt: NDArray
    ) -> Tuple[NDArray, dict]:
        """回転後の辺ベクトルと座標点の作成
    ...
    def _sampling_edge(self, cluster: NDArray, points: dict, sampling_step=0.1) -> list:
    ...
    def _search_for_missing_vertices(
        ...
        def _check_point(
            ...
            if dists[0] < dist_th:
                ...
                # 途中点
                dst_pt = np.array(
                    [sampling_points[inds[0]][0], sampling_points[inds[0]][1], 0.0]
                )
                return dst_pt, t
            else:
                return None
    ...
    def _create_wall_surface(
        ...
        for line_info in line_infos:
            ...
            if line_info.id2 < 0:
                ...
                # 島屋根(穴)の確認
                if (
                    cluster1.parent > self._NO_PARENT_IDX
    ...
    def _check_reverse_wall(self, wall_points: NDArray, normal: NDArray) -> bool:
        """壁面の頂点列の反転確認
    ...
    def _create_ground_surface(
        self, shape: geo.Polygon, ground_height: float
    ) -> FaceInfo:
        """地面作成
    ...
    def _edge_dist_clustering(
        self, edges: NDArray, points: dict, dist_th=0.1, sampling_step=0.01
    ) -> list[NDArray]:
        """辺の距離によるクラスタリング
