...
class Preprocess:
    ...
    def _height_clustering(self, cloud: PointCloud, z_band_width=0.5) -> list:
        ...
        clustering = MeanShift(bandwidth=z_band_width, bin_seeding=True, n_jobs=8).fit(
            zs[..., np.newaxis]
        )
    ...
    def _color_clustering(self, hier_points: list, rgb_band_width=20) -> list:
        """色によるクラスタリング
        ...
        for hier_points_ in hier_points:
            clustering = MeanShift(
                bandwidth=rgb_band_width, bin_seeding=True, n_jobs=8
            ).fit(hier_points_[self._RGB] / 256)
            labels = clustering.labels_
    ...
    def _dbscan(self, hier_clusters: list, eps=0.5, point_th=5, n=20) -> list:
        """DBSCAN
        ...
        for i in range(len(hier_clusters)):
            db = DBSCAN(eps=eps, min_samples=point_th).fit(hier_clusters[i][self._XYZ])
            labels = db.labels_
    ...
    """
        ...
        if len(points_xyz) > 0:
            ...
            inds = nn.radius_neighbors(footprint_points_xy, return_distance=False)
    ...
    def _merge(
        self, hier_clusters_fpnn: list, height_th=2.0, dbscan_eps=0.5, dbscan_point_th=5
    ) -> list:
