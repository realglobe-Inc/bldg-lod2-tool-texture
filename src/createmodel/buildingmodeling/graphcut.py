...
class GraphCut:
    ...
    def _get_distance_to_plane(
        self, points: NDArray, offset: NDArray, alpha: float, beta: float, gamma: float
    ):
        """各点と平面との距離
    ...
    """
        ...
        if len(roof_cloud.get_points()) == 0:
            # 屋根点が取得でき無かった場合
            msg = "{}.{}, {}".format(
                class_name, func_name, ModelingMessage.ERR_MSG_GRAPHCUT_NO_ROOF_POINTS
            )
        ...
        for merged_labels_ in merged_labels:
            ...
            db = DBSCAN(eps=dbscan_eps, min_samples=dbscan_point_th).fit(
                merged_points_xyz[:, 0:2]
            )
            ...
            for i in range(db_labels.max() + 1):
                ...
                new_label_heights.append(
                    np.mean(label_heights[np.unique(merged_points_label[inds])])
                )
                cur_label += 1
        ...
        if len(new_roof_points) == 0:
            ...
            msg = "{}.{}, {}".format(
                class_name,
                func_name,
                ModelingMessage.ERR_MSG_GRAPHCUT_NO_MERGED_ROOF_POINTS,
            )
            raise ModelingException(msg)
