...
from src.createmodel.housemodeling.extra_roof_line.utils.merge_close_vertices import merge_close_vertices
from src.createmodel.housemodeling.extra_roof_line.utils.line_group import LineGroup
...
class ExtraRoofLine:
  ...
  def _get_inner_polygon_ijs_list_after(self, inner_polygon_ijs_list_before):
    ...
    for tmp_inner_polygon_ijs in tmp_inner_polygon_ijs_list_after:
      ...
      for index, inner_polygon_ij in enumerate(tmp_inner_polygon_ijs):
        ...
        line_point_ijs = line_group.get_line_group(inner_polygon_ij, next_inner_polygon_ij)
      ...
      assert new_poly.is_valid, "不正ポリゴンです"
      if not new_poly.is_valid:
        raise CreateModelException(CreateModelMessage.ERR_POLYGON_DIVISION_FAIL)
  ...
  def _to_polygon_xys(self, inner_polygon_ijs_list: list[list[tuple[float, float]]]):
    ...
    ij_polys: list[Polygon] = []
    for tmp_inner_polygon in tmp_inner_polygons:
      polygon_ijs = [vertex_ijs[point_id] for point_id in tmp_inner_polygon]
      assert len(polygon_ijs) >= 3, "不正ポリゴンです"
      if not len(polygon_ijs) >= 3:
        raise CreateModelException(CreateModelMessage.ERR_POLYGON_DIVISION_FAIL)
      ...
      ij_polys.append(poly)

    vertex_xys = [self._ij_to_xy(vertex_ij) for vertex_ij in vertex_ijs]
    xy_polys: list[Polygon] = []
    for tmp_inner_polygon in tmp_inner_polygons:
      polygon_xys = [vertex_xys[point_id] for point_id in tmp_inner_polygon]
      assert len(polygon_xys) >= 3, "不正ポリゴンです"
      if not len(polygon_xys) >= 3:
        raise CreateModelException(CreateModelMessage.ERR_POLYGON_DIVISION_FAIL)
      ...
      xy_polys.append(poly)

    outer_xy_poly = unary_union(xy_polys)
    assert isinstance(outer_xy_poly, Polygon), 'ポリゴン分割に失敗しました'
    if not isinstance(outer_xy_poly, Polygon):
      raise CreateModelException(CreateModelMessage.ERR_POLYGON_DIVISION_FAIL)

    after_outer_polygon_xys = list(outer_xy_poly.exterior.coords[:-1])
    after_polygon_xys_list = [list(poly.exterior.coords[:-1]) for poly in xy_polys]
