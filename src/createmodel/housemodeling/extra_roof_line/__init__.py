...
class ExtraRoofLine:
    ...
    def _has_too_many_noise(self):
        ...
        for polygon_ijs in self._inner_polygon_ijs_list_after:
            layer_number_point_ijs_pair = (
                PolygonDevision.get_layer_number_grid_ijs_pair(
    ...
    def _save_roof_line_with_layer_class_images(
        ...
        # RGBイメージの初期化
        dsm_grid_rgbs_of_roof_line_with_layer_class_as_is = np.full(
            (self._height, self._width, 3), 255, dtype=np.uint8
        )
        dsm_grid_rgbs_of_roof_line_with_layer_class_to_be = np.full(
            (self._height, self._width, 3), 255, dtype=np.uint8
        )
        ...
        for polygon_ijs in inner_polygon_ijs_list:
            ...
            majority_layer_number = PolygonDevision.get_majority_layer_number(
                layer_number_point_ijs_pair
            )
            for layer_number, layer_points_ij in layer_number_point_ijs_pair.items():
                for i, j in layer_points_ij:
                    dsm_grid_rgbs_of_roof_line_with_layer_class_as_is[i, j] = (
                        self._roof_layer_info.get_color(layer_number)
                    )
                    dsm_grid_rgbs_of_roof_line_with_layer_class_to_be[i, j] = (
    ...
    def _get_inner_polygon_ijs_list_after(
        ...
        for polygon_ijs_before in inner_polygon_ijs_list_before:
            ...
            tmp_inner_polygon_ijs_list_after.extend(
                polygon_devision.splited_polygon_ijs_list
            )
            ...
            if polygon_devision.has_splited_polygon:
                ...
                splited_polygon_outer_ijs: list[tuple[float, float]] = (
                ...
                for index, polygon_ij in enumerate(polygon_ijs_before):
                    ...
                    splited_polygon_outer_ij_index = splited_polygon_outer_ijs.index(
                        polygon_ij
                    )
                    next_splited_polygon_outer_ij_index = (
                        splited_polygon_outer_ijs.index(next_polygon_ij)
                    )
                    index_distance = abs(
                        next_splited_polygon_outer_ij_index
                        - splited_polygon_outer_ij_index
                    )

                    if index_distance != 1 or index_distance != (
                        len(splited_polygon_outer_ijs) - 1
                    ):
                        start_index = (splited_polygon_outer_ij_index + 1) % len(
                            splited_polygon_outer_ijs
                        )
                        end_index = next_splited_polygon_outer_ij_index
                        curren_index = start_index

                        while curren_index != end_index:
                            new_polygon_outer_ij = splited_polygon_outer_ijs[
                                curren_index
                            ]
        ...
        if self._debug_mode:
            ...
            for polygon_idx, tmp_inner_polygon_ijs in enumerate(
                ...
                extra_roof_line_poligons_2d.append_faces(
                    BldElementType.ROOF, [polygon_layer_ijs]
                )

            debug_dir = os.path.join("debug", self._id)
            Path(debug_dir).mkdir(parents=True, exist_ok=True)
            extra_roof_line_poligons_2d_path = os.path.join(
                debug_dir, "extra_roof_line_poligons_2d.obj"
            )
            extra_roof_line_poligons_2d.write_file(
                file_path=extra_roof_line_poligons_2d_path
            )
