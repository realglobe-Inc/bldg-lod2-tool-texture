...
class PolygonDevision:
    ...
    def _save_splited_polygons_image(
        ...
        image_layer_splited_polygons_image = np.full(
        ...
        cv2.polylines(
            image_layer_splited_polygons_image,
            polygons_np,
            isClosed=True,
            color=edge_color,
            thickness=1,
        ...
        cv2.imwrite(
            image_layer_splited_polygons_path, image_layer_splited_polygons_image
        )

    def _calculate_slope(
        self, point1_ij: tuple[float, float], point2_ij: tuple[float, float]
    ) -> float:
        """
        2点間の傾きを計算する
