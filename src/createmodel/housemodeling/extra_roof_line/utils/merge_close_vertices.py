...
def merge_close_vertices(
    polygon_ijs_list: list[list[tuple[float, float]]], threshold: float = 1e-5
):
    """
    近い点をマージする
    ...
    change_map: dict[tuple[float, float], tuple[float, float]] = (
        {}
    )  # 変更前後の頂点対応を保持
    ...
    for vertex_ij in vertex_ijs:
        ...
        for merged_vertex_ij in merged_vertice_ijs:
            ...
            if (
                np.linalg.norm(np.array(vertex_ij) - np.array(merged_vertex_ij))
                < threshold
            ):
                # i+j の合計値が小さい方を残す
