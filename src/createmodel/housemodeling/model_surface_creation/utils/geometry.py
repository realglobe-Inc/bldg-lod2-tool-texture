...
class Point:
    ...
    def rotate(self, radian: float, center: Optional[Point] = None) -> Point:
        ...
        moved = self - center
        rotated = Point(
            moved.x * rot.x - moved.y * rot.y, moved.x * rot.y + moved.y * rot.x
        )
...
class Segment:
    ...
    def is_same(self, s: Segment, acceptable_error=AcceptableError) -> bool:
        ...
        """
        return (
            self[0].is_same(s[0], acceptable_error)
            and self[1].is_same(s[1], acceptable_error)
        ) or (
            self[1].is_same(s[0], acceptable_error)
            and self[0].is_same(s[1], acceptable_error)
        )
    ...
    def get_cross_point(
        self, s: Segment, acceptable_error=AcceptableError
    ) -> Optional[Point]:
        """線分との交点を求める
