...
class VerticalObject():
  ...
  def __init__(
    ...
    # テクスチャ画像出力オブジェクト作成
    self._texcollection = DstTextureFile(texture_output_width_max, texture_output_height_max)
  ...
  def select_rooftexture(self):
    ...
    # 屋根用テクスチャ情報オブジェクトを確保
    self._rooftexture = [TextureInfo() for i in range(len(self._vertexroof))]
    ...
    for r_idx, verroof in enumerate(self._vertexroof):
      ...
      for tex in tex_collection.src_texture:
        if tex.ref_image_index == set_idx:
          self._rooftexture[r_idx].texver = tex.append_texturecoord(roof_coord)
          self._rooftexture[r_idx].texinfo = tex
          self._rooftexture[r_idx].polarea = max_area

          refimg = self._photolist[set_idx].filename
          Log.output_log_write(LogLevel.DEBUG, ModuleType.PASTE_TEXTURE, 'roof refImage:' + refimg)
  ...
  def select_walltexture(self):
    ...
    # 壁面用テクスチャ情報オブジェクトを確保
    self._walltexture = [TextureInfo() for i in range(len(self._vertexwall))]
    ...
    for w_idx, wall in enumerate(self._vertexwall):
      ...
      for i in range(self._photonum):
        ...
        # 陰面判定
        if self._judge_hiddensurface(wall, wall_imgPos, roof_imgPos, r_common):
          # 陰面あり
          continue
      ...
      for tex in tex_collection.src_texture:
        if tex.ref_image_index == set_idx:
          self._walltexture[w_idx].texver = tex.append_texturecoord(set_wall_imgpos)
          self._walltexture[w_idx].texinfo = tex
          self._walltexture[w_idx].polarea = max_area

          refimg = self._photolist[set_idx].filename
          Log.output_log_write(LogLevel.DEBUG, ModuleType.PASTE_TEXTURE, f'wall refImage: {refimg}')
  ...
  def _judge_hiddensurface(self, wall, wall_imgpos, roof_imgpos, r_common):
    ...
    for num, w_ver in enumerate(wall_around):
      ...
      if prev_flag == FaceInfo.NOT_ROOF and next_flag == FaceInfo.ROOF:
        # 屋根面→屋根面以外
        tuple_equal = np.where(np.array_equal(ary, wall_imgpos[w_num]) for ary in np.array(roof_imgpos))
        imgpos_chk = self._is_occluded_byself(tuple_equal[0][0], prev_ver, roof_imgpos)

      elif prev_flag == FaceInfo.ROOF and next_flag == FaceInfo.NOT_ROOF:
        # 屋根面以外→屋根面
        tuple_equal = np.where(np.array_equal(ary, prev_ver) for ary in np.array(roof_imgpos))
        imgpos_chk = self._is_occluded_byself(tuple_equal[0][0], wall_imgpos[w_num], roof_imgpos)
...
class DstTextureFile():
  """出力テクスチャファイルクラス
  """
  def __init__(self, texture_output_width_max: Union[float], texture_output_height_max: Union[float]):
    """コンストラクタ
    Args:
      texture_output_width_max (int): テクスチャー横幅最大値(4096まで設定可)
      texture_output_height_max (int): テクスチャー縦幅最大値(4096まで設定可)
  ...
  def output_texture(self, outputpath: str, image_format: str):
    ...
    for srcTex in self.src_texture:
      ...
      img = Cv2Japanese.imread(os.path.join(srcTex.ref_image.photodir, srcTex.ref_image.filename))
      for texver in srcTex.texcoord:
        min_x, min_y, polygon_w, polygon_h, output_margin = get_texpoly_bbox(
            texver, img.shape[1], img.shape[0]
        )
