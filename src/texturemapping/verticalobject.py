...
class VerticalObject:
        ...
        for r_idx, verroof in enumerate(self._vertexroof):
            ...
            for i in range(self._photonum):
                ...
                for j, ver in enumerate(verroof):
                    roof_valid[j] = self._photolist[i].get_imagepos(ver, tex_coord[j])
                    if roof_valid[j]:
                        in_count += 1
        ...
        for w_idx, wall in enumerate(self._vertexwall):
            ...
            for i in range(self._photonum):
                ...
                for num, r_ver in enumerate(self._vertexroof[roof_idx]):
                    if not self._photolist[i].get_imagepos(r_ver, roof_imgPos[num]):
                        # 指定範囲は画像外
                        imgpos_chk = False
                        break
                ...
                for num, w_ver in enumerate(wall):
                    if not self._photolist[i].get_imagepos(w_ver, wall_imgPos[num]):
                        # 指定範囲は画像外
                        imgpos_chk = False
                        break
        ...
        for num, w_ver in enumerate(wall_around):
            ...
            if prev_flag == FaceInfo.NOT_ROOF and next_flag == FaceInfo.ROOF:
                ...
                tuple_equal = np.where(
                    np.array_equal(ary, wall_imgpos[w_num])
                    for ary in np.array(roof_imgpos)
                )
                ...
                # 屋根面以外→屋根面
                tuple_equal = np.where(
                    np.array_equal(ary, prev_ver) for ary in np.array(roof_imgpos)
                )
                imgpos_chk = self._is_occluded_byself(
    ...
    def output_texture(
        self, objdir: str, outputdir: str, mtl_file_name: str, image_format: str
    ):
        """テクスチャ画像・情報の出力
        ...
        ret = self._texcollection.output_texture(
            os.path.join(outputdir, self._obj_filename), image_format
        )
        ...
        if ret:
            ...
            img_path = os.path.join(
                pathlib.Path(outputdir).name, self._obj_filename + "." + image_format
            )
    ...
    def output_optional_obj(
        self, objdir: str, texture_dir: str, mtl_file_name: str, image_format: str
    ):
...
class DstTextureFile:
    ...
    def output_texture(self, outputpath: str, image_format: str):
        ...
        def get_texpoly_bbox(texver, img_width, img_height):
            ...
            if (
                (0 <= min_ver[0] - self._outputmargin)
        ...
        for srcTex in self.src_texture:
            img = Cv2Japanese.imread(
                os.path.join(srcTex.ref_image.photodir, srcTex.ref_image.filename)
            )
            for texver in srcTex.texcoord:
                _, _, polygon_w, polygon_h, _ = get_texpoly_bbox(
