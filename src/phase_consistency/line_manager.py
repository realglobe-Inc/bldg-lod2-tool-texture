...
class LineInfo:
    ...
    def is_pair_line(self, other) -> bool:
        ...
        if not isinstance(other, LineInfo):
            return False
        if GeoUtil.is_same_point(self.pos1, other.pos2) and GeoUtil.is_same_point(
            self.pos2, other.pos1
        ):
            return True
        return False
    ...
    def get_polygon_set(self, line_list: list, polygon_list: list):
        ...
        if same_list_len == 0:
            ...
            # 複数ある場合、内角が小さいものを選択
            min_angle = GeoUtil.interior_angle(
                self.pos1, self.pos2, line_list[same_list[0]].pos1
            )
            index = same_list[0]
            for i in range(1, same_list_len):
                angle = GeoUtil.interior_angle(
                    self.pos1, self.pos2, line_list[same_list[i]].pos1
                )
                if angle < min_angle:
                    index = same_list[i]
                    min_angle = angle
...
class LineManager:
    ...
    @staticmethod
    def hash_val(line: LineInfo) -> int:
        """ハッシュテーブル用のハッシュ値算出
    ...
    @staticmethod
    def _delete_overlap_on_same_line(line1: LineInfo, line2: LineInfo) -> tuple:
        """同一線上の重畳部分除外処理
