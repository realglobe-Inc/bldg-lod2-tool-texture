...
class ErrorInfo:
    ...
    _error_str_dict = {
        ErrorType.DOUBLE_POINT: "Double Points is detected.",
        ErrorType.SELF_INTERSECTION: "Self intersecting " "polygon is detected.",
        ErrorType.FACE_INTERSECTION: "Self intersecting " "faces are detected.",
...
class ResultInfo:
    ...
    def output_obj_file(self, dir_path: str):
        ...
        for i, err_info in enumerate(self._err_list):
            if len(err_info.pos_list) > 2:
                obj_info = ObjInfo()
                obj_info.append_point_list(BldElementType.ROOF, err_info.pos_list)
