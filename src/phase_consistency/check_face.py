...
from .line_manager import TriangleInfo, LineManager
from ..util.face_info import FaceInfo, IndexInfo
from ..util.obj_info import ObjInfo, BldElementType
...
class CheckFace:
    ...
    def check_double_point(self) -> TestResultType:
        ...
        logger.debug(f"len(self._face.index) = {len(self._face.indices)}")
        logger.debug(
        ...
        delete_list = list()
        for i in range(len(self._face.indices)):
            cur_pos = self._v_list.get_pos(self._face.indices[i].pos)
            pre_pos = self._v_list.get_pos(self._face.indices[i - 1].pos)
        ...
        if len(delete_list):
            if len(self._face.indices) <= 3:
                ret = TestResultType.AUTO_CORRECTION_FAILURE
            else:
                delete_list.reverse()
                for i in delete_list:
                    del self._face.indices[i]
    ...
    def check_intersection(self) -> bool:
        ...
        # Point リスト作成
        p_list = []
        for index in self._face.indices:
...
class CheckFaces:
    ...
    def _get_element_type(self, point_list: list) -> BldElementType:
        ...
        under_direction_vec = np.array([0.0, 0.0, -1.0])
        ...
        if GeoUtil.is_same_value(normal_length, 0.0):
            ...
            dot = GeoUtil.dot(normal_vec, under_direction_vec)
            angle = abs(math.degrees(math.acos(dot)))
