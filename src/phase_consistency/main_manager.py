...
from .check_face import CheckFace, CheckFaces, TestResultType
from .result_info import ErrorType
from .result_info import ResultInfo, StatusType
from ..util.city_gml_info import CityGmlManager
from ..util.config import Config
from ..util.log import Log, ModuleType, LogLevel
from ..util.obj_info import ObjInfos, ObjInfo
from ..util.param_manager import ParamManager
from ..util.result_type import ProcessResult, ResultType
...
class MainManager:
    ...
    _module_name = "CheckPhaseConsistency Module"
    ...
    def __init__(self, param_manager: ParamManager):
        ...
        self._input_folder = Config.OUTPUT_MODEL_OBJ_DIR  # 入力フォルダパス
        self._output_folder = Config.OUTPUT_PHASE_OBJ_DIR  # 出力フォルダパス
    ...
    def check_and_correction(self, buildings: list[CityGmlManager.BuildInfo]):
        """位相一貫性検査/補正処理実行
    ...
    def _output_log_file_summary(self):
        ...
        Log.output_log_write(LogLevel.INFO, ModuleType.CHECK_PHASE_CONSISTENCY, message)
        ...
        for f_key, f_value in obj_info.faces_list.items():
            ...
            for face in f_value.faces:
                ...
                if not check_face.check_zero_area():
                    ...
                    # エラーあり
                    result_info.add_err(ErrorType.ZERO_AREA, check_face.err_list)
    ...
    @staticmethod
    def _check_solid(mesh, result_info: ResultInfo, build: CityGmlManager.BuildInfo):
        """ソリッド閉合検査/補正
