...
class GeoUtil:
    ...
    @classmethod
    def min(cls, val1, val2):
        if val1 < val2:
            return val1
        return val2

    @classmethod
    def max(cls, val1, val2):
        if val1 > val2:
            return val1
        return val2

    @classmethod
    def get_point_str(cls, pos: Point):
        return "(" + str(pos.x) + ", " + str(pos.y) + ", " + str(pos.z) + ")"

    @classmethod
    def is_same_value(cls, value1: float, value2: float) -> bool:
        """浮動小数点の同一比較判定
    ...
    @classmethod
    def float_is_zero(cls, value: float) -> bool:
        """浮動小数点が0であるかチェック
    ...
    @classmethod
    def normalize(cls, vec: NDArray) -> NDArray:
        """単位ベクトルに正規化
    ...
    @classmethod
    def dot(cls, vec1: NDArray, vec2: NDArray) -> float:
    ...
    @classmethod
    def get_normal(cls, p_list: list) -> NDArray:
        """面の法線ベクトルを算出
        ...
        return cls.normalize(normal)

    @classmethod
    def interior_angle(cls, p1: Point, p2: Point, p3: Point) -> float:
        """3 頂点が作る角度算出
    ...
    @classmethod
    def conv_2d(cls, p_list: list) -> list:
        """3次元の面を 2次元に変換
        ...
        y_vec = cls.normalize(y_vec)
        x_vec = np.cross(y_vec, z_vec)
        x_vec = cls.normalize(x_vec)
    ...
    @classmethod
    def get_line_list(cls, p_list: list) -> list:
        """線分リストを取得する
    ...
    @classmethod
    def is_cross_or_touch(cls, p_list: list, err_list: list = None) -> bool:
        """面の自己交差/接触判定
        ...
        # 平面に投影
        xy_list = cls.conv_2d(p_list)

        # 自己交差チェック
        err_index = []
        if cls.is_cross_2d(xy_list, err_index):
            for i in err_index:
                err_list.append(p_list[i])

        # 自己接触チェック
        err_index = []
        if cls.is_touch_2d(xy_list, err_index):
            for i in err_index:
                err_list.append(p_list[i])
    ...
    @classmethod
    def is_cross_2d(cls, p_list: list, err_list: list) -> bool:
        """面の自己交差判定 (2次元)
        ...
        line_list = cls.get_line_list(p_list)
    ...
    @classmethod
    def is_touch_2d(cls, p_list: list, err_list: list) -> bool:
        """自己接触判定
        ...
        for i in range(len(p_list)):
            ...
            for j in range(0, len(p_list)):
                ...
                if p.distance(line_list[i]) <= cls.delta_threshold:
                    r_flag = True
                    err_list.append(j)
    ...
    @classmethod
    def is_convex_polygon(cls, p_list: list) -> bool:
        """凸多角形かどうかを判定
        ...
        for i in range(pos_len):
            ...
            for j in range(2, pos_len):
                ...
                v01 = cls.normalize(v01)
                v02 = cls.normalize(v02)

                normal = np.cross(v02, v01)
                normal = cls.normalize(normal)
    ...
    @classmethod
    def is_zero_area(cls, p_list: list) -> bool:
        """面積 0 のポリゴンかどうか判定
    ...
    @classmethod
    def divide_triangle_for_convex_polygon(cls, p_list: list) -> list:
        """凸ポリゴンを三角形に分割
        ...
        while len(vertex_list) >= 3:
            ...
            triangle_list = [
                vertex_list[prev_index],
                vertex_list[cur_index],
                vertex_list[post_index],
            ]
    ...
    def divide_triangle_for_concave_polygon(
        cls, p_list: list, check_intersect: bool = True
    ) -> list:
        """凹ポリゴンを三角形に分割

        Args:
            p_list (list<Point>): 入力ポリゴンの頂点リスト
            check_intersect (bool): 自己交差・接触チェックするか
        ...
        while len(xy_list) > 3:
            ...
            if loop_ct > vertex_num * 2 + 10:
                ...
                if not check_intersect:
                    multi_point_list.append(triangle_list)
                elif not cls.is_cross_or_touch(triangle_list):
                    multi_point_list.append(triangle_list)
            ...
            prev_vec = cls.normalize(prev_vec)
            post_vec = cls.normalize(post_vec)
            ...
            if check_intersect and cls.is_cross_or_touch(triangle_list):
                cur_index += 1
                continue
        ...
        # 最後の三角形を追加
        if len(xy_list) == 3:
            triangle_list = [vertex_list[0], vertex_list[1], vertex_list[2]]
            # 自己交差チェック
            if not check_intersect:
                multi_point_list.append(triangle_list)
            elif not cls.is_cross_or_touch(triangle_list):
    ...
    @classmethod
    def divide_triangle(cls, p_list: list, check_intersect: bool = True) -> list:
        """面を三角形に分割
        ...
        if cls.is_convex_polygon(p_list):
            return cls.divide_triangle_for_convex_polygon(p_list)
        else:
            return cls.divide_triangle_for_concave_polygon(p_list, check_intersect)

    @classmethod
    def get_min(cls, pos1: Point, pos2: Point) -> Point:
        ...
        Returns:
            Point: 最小座標値
        """
        pos_x = cls.min(pos1.x, pos2.x)
        pos_y = cls.min(pos1.y, pos2.y)
        pos_z = cls.min(pos1.z, pos2.z)

        return Point(pos_x, pos_y, pos_z)

    @classmethod
    def get_max(cls, pos1: Point, pos2: Point) -> Point:
        ...
        Returns:
            Point: 最大座標値
        """
        pos_x = cls.max(pos1.x, pos2.x)
        pos_y = cls.max(pos1.y, pos2.y)
        pos_z = cls.max(pos1.z, pos2.z)

        return Point(pos_x, pos_y, pos_z)

    @classmethod
    def get_minmax(cls, p_list: list) -> tuple:
        ...
        for i in range(1, point_num):
            p_min = cls.get_min(p_list[i], p_min)
            p_max = cls.get_max(p_list[i], p_max)

        return p_min, p_max

    @classmethod
    def is_same_point(cls, pos1: Point, pos2: Point) -> bool:
        """2 頂点が同一座標かどうかを判定
    ...
    @classmethod
    def is_zero(cls, length: float) -> bool:
        if abs(length) < 0.01:
            return True
        return False

    @classmethod
    def is_in_triangle(cls, normal: NDArray, pos: NDArray, triangle: list) -> bool:
        """頂点が三角形内部にあるかどうかを判定
    ...
    def is_cross_triangle(
        cls, triangle_info1: list, triangle_info2: list, cross_point_list: list
    ) -> bool:
        """2つの三角形の交差判定
           交差ありの場合には交点座標を返す

        Args:
            triangle_info1 (list<TriangleInfo>): 三角形1
            triangle_info2 (list<TriangleInfo>): 三角形2
        ...
        for i in range(3):
            for j in range(3):
                if cls.is_same_point(triangle1[i], triangle2[j]) and cls.is_same_point(
                    triangle1[(i + 1) % 3], triangle2[(j + 1) % 3]
                ):
                    return False
                if cls.is_same_point(
                    triangle1[i], triangle2[(j + 1) % 3]
                ) and cls.is_same_point(triangle1[(i + 1) % 3], triangle2[j]):
                    return False
        ...
        v1_01 = cls.normalize(v1_01)
        v1_02 = cls.normalize(v1_02)

        v1_normal = np.cross(v1_01, v1_02)
        v1_normal = cls.normalize(v1_normal)
        ...
        for i in range(3):
            ...
            # 三角形の内側にあるかチェック
            if cls.is_in_triangle(v1_normal, cross_pos, triangle1):
    ...
    @classmethod
    def is_on_same_line(cls, p1: Point, p2: Point, q1: Point, q2: Point) -> bool:
