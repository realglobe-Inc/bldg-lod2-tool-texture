...
def find_hole_bridge(hole, outer_node):
    ...
    while p != stop:
        ...
        if hx >= p.x >= mx and point_in_triangle(
            hx_or_qx, hy, mx, my, qx_or_hx, hy, p.x, p.y
        ):

            tan = abs(hy - p.y) / (hx - p.x)  # tangential

            if (tan < tan_min or (tan == tan_min and p.x > m.x)) and locally_inside(
                p, hole
            ):
                m = p
                tan_min = tan
...
def is_valid_diagonal(a, b):
    return (
        a.next.i != b.i
        and a.prev.i != b.i
        and not intersects_polygon(a, b)
        and locally_inside(a, b)
        and locally_inside(b, a)
