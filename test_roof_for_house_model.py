...
def is_point_in_polygon(
    ...
    holes: Union[
        list[list[list[float]]], list[NDArray[np.float_]], NDArray[np.float_]
    ] = [],
):
...
def get_polygon_vertices_with_holes(
    polygon_vertices: list[list[float]], holes_vertices: list[list[list[float]]]
):
    ...
    for holes_vertice in holes_vertices:
        ...
        holes_coords = [
            [list(coord) for coord in hole.coords]
            for hole in roof_floor1_polygon.interiors
        ]
        all_vertices = exterior_coords + [
            coord for hole in holes_coords for coord in hole
        ]
...
for roof_floor2_outer_ring_vertices in roof_floor2_outer_rings_vertices:
    ...
    info.append_faces(
        BldElementType.ROOF, [reorder_points(roof_floor2_outer_ring_vertices)]
    )


# 穴のある多角形データ
roof_floor1_outer_ring_xy = np.array(
    roof_floor1_wall_points[:, :2], dtype=np.float_
)  # 外部の境界
...
roof_floor1_holes_vertices = [
    np.hstack([hole, np.full((hole.shape[0], 1), roof_floor1_height)])
    for hole in roof_floor1_holes_xy
]

roof_floor1_vertices = get_polygon_vertices_with_holes(
    roof_floor1_outer_ring_vertices, roof_floor1_holes_vertices
)
...
for roof_floor2_outer_ring_vertices in roof_floor2_outer_rings_vertices:
    for i in range(len(roof_floor2_outer_ring_vertices)):
        ...
        wall_vertices = [
            ...
            np.append(
                roof_floor2_outer_ring_vertices[next_i][:2], roof_floor1_height
            ).tolist(),
            np.append(
                roof_floor2_outer_ring_vertices[i][:2], roof_floor1_height
            ).tolist(),
        ]
