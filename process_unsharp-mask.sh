...
input_dir=$(realpath -m "${INPUT_DIR:-${base_output_dir}/output_deblurgan}")
output_dir=$(realpath -m "${OUTPUT_DIR:-${base_output_dir}/output_unsharp_mask}")

# テクスチャシャープ化ツールのフォルダーに移動
cd "${project_dir}/tools/UnsharpMask"

rm -rf "${output_dir}/*"

. "./$(basename $PWD)/bin/activate"
python UnsharpMask.py -i "${input_dir}" -o "${output_dir}"
deactivate
