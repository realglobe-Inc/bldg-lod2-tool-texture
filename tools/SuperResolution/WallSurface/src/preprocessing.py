...
class PreProcessing:
    def __init__(self, logger, overlap=0.0, size=256, pixel_per_meter=0.16, z_threshold=0.02, lower_limit=32,
                 upper_limit=1024):
        self.logger = logger
    ...
    def main_step(self, obj_path: pathlib.Path, output_dir: pathlib.Path):
        ...
        cut_logs = []
        cut_log_paths = []
        seitaika_info, seitaika_figs, roof_info = seitaika_main(self.logger, obj_path, output_dir, self.pixel_per_meter,
                                                                self.z_threshold)
