...
class BaseModel(ABC):
    ...
    def __init__(self, cfg, param):
        ...
        """
        self.cfg_direction = cfg["direction"]
        self.cfg_dataset = cfg["dataset"]
        self.cfg_model = cfg["model"]

        self.isTrain = self.cfg_model["isTrain"]
        self.device = (
            torch.device("cuda:0") if param == "cuda" else torch.device("cpu")
    ...
    def setup(self, cfg):
        ...
        if self.isTrain:
            self.schedulers = [
                networks.get_scheduler(optimizer, self.cfg_train)
                for optimizer in self.optimizers
            ]
        if not self.isTrain or self.cfg_model["continue_train"]:
            load_suffix = (
                "iter_%d" % self.cfg_train["load_iter"]
                if self.cfg_train["load_iter"] > 0
                else self.cfg_train["epoch"]
            )
