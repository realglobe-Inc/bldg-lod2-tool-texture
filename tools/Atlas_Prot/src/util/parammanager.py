...
class ParamManager:
  ...
  def read(self, file_path) -> list[ChangeParam]:
    ...
    self.input_gml_folder_path = fix_relative_path(
        Path(jsonLoad[self.KEY_FILE_PATH][self.KEY_INPUT_GML_FOLDER_PATH]).expanduser()
    )
    self.output_gml_folder_path = fix_relative_path(
        Path(jsonLoad[self.KEY_FILE_PATH][self.KEY_OUTPUT_GML_FOLDER_PATH]).expanduser()
    )
    ...
    if (type(self.input_gml_folder_path) is not str or not self.input_gml_folder_path):
      # 文字列ではない or 空文字の場合
      raise Exception(ParamManager.KEY_FILE_PATH.KEY_INPUT_GML_FOLDER_PATH + ' is invalid.')
    if not os.path.isdir(self.input_gml_folder_path):
      # 入力CityGMLフォルダが存在しない場合
      raise Exception(ParamManager.KEY_FILE_PATH.KEY_INPUT_GML_FOLDER_PATH + ' not found.')

    if (type(self.output_gml_folder_path) is not str or not self.output_gml_folder_path):
      # 文字列ではない or 空文字の場合
      raise Exception(ParamManager.KEY_FILE_PATH.KEY_OUTPUT_GML_FOLDER_PATH + ' is invalid.')
    ...
    if (type(self.background_color) is not int or not (0 <= self.background_color <= 255)):
      raise Exception(ParamManager.KEY_FILE_PATH.KEY_BACKGROUND_COLOR + ' is invalid.')
