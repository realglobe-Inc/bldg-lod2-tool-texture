...
class plparser:
    ...
    def addPath(self, path):  # path to CityGML
        ...
        locations.extend(
            [
                plobj.getLocationFromFilename(filename, True)
    ...
    def loadFiles(
        ...
        if kind == plobj.BLDG or kind == plobj.ALL:
            if location < 0:
                ...
                for filename in self.filenames_bldg:
                    if (
                        plobj.getLocationFromFilename(filename, False) == location
                        or plobj.getLocationFromFilename(filename, True) == location
                    ):
        ...
        if bLoadCache:
            ...
            for f in filenames_bldg:
                if (options.div6toQuarter is not None) and (
                    options.div6toQuarter != plobj.get6QuarterFromFilename(f)
                ):
                    continue
                obj = plbldg()
