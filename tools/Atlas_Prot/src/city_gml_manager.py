...
class CityGmlManager():
  ...
  def input_citygml(self):
    ...
    for index, gml_file in enumerate(self.citygml_names):
      ...
      pbar = tqdm(
          total=len(apps),
          desc=f"Processing({gml_file})({index + 1}/{len(self.citygml_names)})",
          unit="item",
      )
      for elem1 in apps:
        # 建物毎に処理
        ...
        for elem2 in target:
          ...
          building.add_polygon_info(uri, ring, arrays_r, [building.input_image_width, building.input_image_height], self._pm.extent_pixel)
