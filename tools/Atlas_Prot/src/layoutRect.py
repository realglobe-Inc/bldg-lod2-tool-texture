...
class LayoutTexture():
  ...
  def layout_texture_main(self):
    ...
    for citygml_info in self._citygml_infos:
      ...
      for building in citygml_info.buildings:
        # 指定棟数ごとに処理
        if (
            ((self._pm.output_width == building.input_image_width) and (self._pm.output_height == building.input_image_height))
            or ((self._pm.output_width < building.input_image_width) or (self._pm.output_height < building.input_image_height))
        ):
          output_image_path = os.path.join(self._pm.output_gml_folder_path, building.input_image_path)
          ...
          shutil.copyfile(os.path.join(self._pm.input_gml_folder_path, building.input_image_path), output_image_path)
      ...
      for building in citygml_info.buildings:
        ...
        if range < len(self._new_ranges):
          if 1 == bld_count - range_count:
            ...
            while val_w < out_w:
              val_w *= 2
            out_w = val_w
            output = np.full((out_h, out_w, 3), self._pm.background_color, dtype='uint8')

            out_path = building.input_image_path  # 先頭画像のパスを使用

        # オリジナル画像のオープン
        img = Cv2Japanese.imread(os.path.join(self._pm.input_gml_folder_path, building.input_image_path))
        ...
        for poly in building.polygon_infos:
          ...
          if poly.coord_ring == temp.rectID:
            ...
            back = np.full((polygon_h, polygon_w, 3), self._pm.background_color, dtype='uint8')
            ...
            cv2.fillPoly(
                mask, [poly_ver.astype(np.int64)], color=(255, 255, 255),
            )
            cv2.polylines(
                mask, [poly_ver.astype(np.int64)], isClosed=True, color=(255, 255, 255)
            )
            ...
            # 貼り付け先の座標に変換
            uv_set = np.array([building.output_image_width, building.output_image_height])
            # xy_set = (arrays_r
        ...
        if range < len(self._new_ranges):
          if self._new_ranges[range] == bld_count - range_count:
            ...
            image_path = os.path.join(self._pm.output_gml_folder_path, building.output_image_path)
  ...
  def _set_range(self, citygml_info: CityGmlInfo):
    ...
    for i, building in enumerate(citygml_info.buildings, 1):
      if ((self._pm.output_width == building.input_image_width) and (self._pm.output_height == building.input_image_height)) or ((self._pm.output_width < building.input_image_width) or (self._pm.output_height < building.input_image_height)):
        continue
  ...
  def _pack_blf2(self):
    ...
    if minX >= NMAX or minY >= NMAY:
      ...
      if (len(self._input_rect_ary) == trgIdx + 1) or (self._input_rect_ary[trgIdx + 1].imgPath != trgRect.imgPath):
        # 建物一棟分の最後
