...
def change(input: str, output: str, format: str) -> None:
    """
    ...
    :type output: str
    :param format: `imageURI`の古い拡張子を置き換えるための新しいファイル拡張子。
    :type format: str
        ...
        for surface_data_member in appearance.findall("app:surfaceDataMember", namespaces):
            ...
            if parameterized_texture is not None:
                ...
                if image_uri is not None and image_uri.text:
                    image_uri.text = str(Path(image_uri.text).with_suffix(f'.{format}'))
...
def main():
    ...
    parser.add_argument(
      '--format', type=str, default='png', help='Converted image extension'
    )

    args = parser.parse_args()

    # 実行
    change(args.input, args.output, args.format)
